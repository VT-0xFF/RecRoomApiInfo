	...
	public class PlayerTeamMappingNode : PostEvalActionNode
	{
		...
		protected override void _EvaluateNode(bool isMasterClient)
		{
			...
			case PlayerIdMapModes.PlayerToTeam:
				EvaluatePlayerToTeam();
				break;
			case PlayerIdMapModes.TeamToPlayer:
				EvaluateTeamToPlayer();
				break;
			case PlayerIdMapModes.SetPlayerTeam:
				EvaluateSetPlayerTeam();
				break;
			}
		}

		private void EvaluatePlayerToTeam()
		{
			int inputSignal = GetInputSignal(InputSlot.R);
			if (inputSignal != 0 && this.PlayerToTeamIndex != null)
			{
				int teamId = 0;
				int teamIndex = 0;
				try
				{
					this.PlayerToTeamIndex(inputSignal, out teamId, out teamIndex);
					return;
				}
				catch (Exception e)
				{
				...
				{
					SetContinuousOutput(OutputSlot.G, teamId);
					SetContinuousOutput(OutputSlot.B, teamIndex);
				}
			}
			SetContinuousOutput(OutputSlot.G, 0);
			SetContinuousOutput(OutputSlot.B, 0);
		}

		private void EvaluateTeamToPlayer()
		{
			int inputSignal = GetInputSignal(InputSlot.G);
			int inputSignal2 = GetInputSignal(InputSlot.B);
			if (inputSignal != 0 && inputSignal2 != 0 && this.TeamIndexToPlayer != null)
			{
				int playerId = 0;
				try
				{
					this.TeamIndexToPlayer(inputSignal, inputSignal2, out playerId);
					return;
				}
				catch (Exception e)
				{
				...
				{
					SetContinuousOutput(OutputSlot.R, playerId);
				}
			}
			SetContinuousOutput(OutputSlot.R, 0);
		}

		private void EvaluateSetPlayerTeam()
		{
...
namespace RecRoom.Circuits.Protobuf
{
	...
	public static class CircuitNodeDataReflection
	{
		...
		static CircuitNodeDataReflection()
		{
			...
			{
				new GeneratedClrTypeInfo(typeof(CircuitNodeData), CircuitNodeData.Parser, new string[11]
				{
...
namespace RecRoom.Circuits.PlayerStats
{
	...
	public static class FriendlyCircuitPlayerStats
	{
		...
		public static Dictionary<string, int> FriendlyCircuitPlayerStatsDict
		{
			get
			{
				...
				{
					string key = ((circuitPlayerStat != CircuitPlayerStat.Player_Health) ? $"Stat {(int)(circuitPlayerStat - 1000)}" : "Health");
	...
	public class PlayerGetStatNode : SourceNodeBase
	{
		...
		protected override void _EvaluateNode(bool isMasterClient)
		{
			...
			try
			{
				val = this.GetPlayerStat(PlayerStat, inputSignal);
			}
			catch (Exception e)
			{
				CircuitDebug.LogException(e);
			...
			{
				SetMomentaryOutput(OutputSlot.R, val);
			}
		}
...
namespace RecRoom.Circuits.GameStats
{
	...
	public class GameGetScoreNode : SourceNodeBase
	{
		...
		protected override void _EvaluateNode(bool isMasterClient)
		{
			...
			try
			{
				val = this.GetTeamScore(inputSignal);
			}
			catch (Exception e)
			{
				CircuitDebug.LogException(e);
