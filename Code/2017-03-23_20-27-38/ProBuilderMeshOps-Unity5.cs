...
namespace ProBuilder2.MeshOperations
{
	public static class pbMeshOps
	{
		...
		public static bool Extrude(this pb_Object pb, pb_Face[] faces, ExtrudeMethod method, float extrudeDistance, out pb_Face[] appendedFaces)
		{
			...
			pb_Edge[][] array = (flag ? new pb_Edge[1][] { pbMeshUtils.GetPerimeterEdges(dictionary, faces).ToArray() } : faces.Select((pb_Face pb_Face6) => pb_Face6.edges).ToArray());
			...
			for (int num5 = 0; num5 < array.Length; num5++)
			{
				...
				for (int num6 = 0; num6 < array[num5].Length; num6++)
				{
					...
					{
						num9 = pb_Math.Secant(Vector3.Angle(vector, to) * ((float)System.Math.PI / 180f)) * extrudeDistance;
						num10 = pb_Math.Secant(Vector3.Angle(vector, to2) * ((float)System.Math.PI / 180f)) * extrudeDistance;
					}
					ref Vector3 reference = ref array6[pb_Edge.x];
					reference = to.normalized * num9;
					ref Vector3 reference2 = ref array6[pb_Edge.y];
					reference2 = to2.normalized * num10;
					list.Add(new Vector3[4]
					{
						vertices[pb_Edge.x],
						vertices[pb_Edge.y],
						vertices[pb_Edge.x] + array6[pb_Edge.x],
						vertices[pb_Edge.y] + array6[pb_Edge.y]
					});
			...
			int num11 = 0;
			int num12 = 0;
			for (; num11 < array5.Length; num11++)
			{
				for (int num13 = 0; num13 < array5[num11].Count; num13 += 2)
				{
					array5[num11][num13].y = appendedFaces[num12].indices[2];
					array5[num11][num13 + 1].y = appendedFaces[num12++].indices[4];
				}
			}
			...
			{
				for (int num15 = 0; num15 < array5[num14].Count - 1; num15++)
				{
					int x = array5[num14][num15].x;
					for (int num16 = num15 + 1; num16 < array5[num14].Count; num16++)
					{
						if (array5[num14][num16].x == x)
						{
							dictionary2[array5[num14][num15].y] = dictionary2[array5[num14][num16].y];
							break;
						}
					}
				}
			...
			if (flag)
			{
				...
				foreach (pb_Face pb_Face4 in array4)
				{
					...
					int[] array7 = distinctIndices;
					foreach (int num18 in array7)
					{
						int num19 = sharedIndices2.IndexOf(num18);
						for (int num20 = 0; num20 < array3.Length; num20++)
						{
							for (int num21 = 0; num21 < array5[num20].Count; num21++)
							{
								if (num19 == array5[num20][num21].x)
								{
									dictionary2[num18] = dictionary2[array5[num20][num21].y];
									break;
								}
							}
						}
			...
			else
			{
				...
				{
					int[] array7 = pb_Face.AllTrianglesDistinct(array2[num22]);
					foreach (int key in array7)
					{
						int old_si_index = dictionary[key];
						int num23 = array5[num22].FindIndex((pb_Edge pb_Edge2) => pb_Edge2.x == old_si_index);
						if (num23 >= 0)
						{
							int y = array5[num22][num23].y;
			...
			foreach (pb_Face pb_Face5 in array4)
			{
				...
				Vector3 to3 = (flag ? Vector3.zero : vector2);
				int[] array7 = pb_Face5.distinctIndices;
				foreach (int num25 in array7)
				{
					if (flag)
					{
						to3 = Norm(sharedIndices[dictionary[num25]], all, normals);
						if (method == ExtrudeMethod.FaceNormal)
						{
							num24 = pb_Math.Secant(Vector3.Angle(vector2, to3) * ((float)System.Math.PI / 180f)) * extrudeDistance;
						}
					}
					vertices[num25] += to3.normalized * num24;
				}
			}
	...
	public static class pb_Extrude
	{
		...
		private static bool ExtrudeAsGroups(pb_Object pb, pb_Face[] faces, bool compensateAngleVertexDistance, float distance)
		{
			...
			foreach (KeyValuePair<int, pb_Tuple<Vector3, Vector3, List<int>>> item5 in dictionary6)
			{
				...
				vector2.Normalize();
				float num4 = (compensateAngleVertexDistance ? pb_Math.Secant(Vector3.Angle(vector2, item5.Value.Item2) * ((float)System.Math.PI / 180f)) : 1f);
		...
		public static HashSet<pb_Face> FloodSelection(pb_Object pb, IList<pb_Face> faces, float maxAngleDiff)
		{
			...
			for (int i = 0; i < wingedEdges.Count; i++)
			{
				if (!hashSet2.Contains(wingedEdges[i].face) && hashSet.Contains(wingedEdges[i].face))
				{
					...
					Flood(pb, wingedEdges[i], (maxAngleDiff > 0f) ? pb_Math.Normal(pb, wingedEdges[i].face) : Vector3_Zero, maxAngleDiff, hashSet2);
				}
			}
			return hashSet2;
		}
