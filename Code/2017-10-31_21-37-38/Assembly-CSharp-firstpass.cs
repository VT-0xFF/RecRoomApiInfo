...
namespace UnityStandardAssets.CinematicEffects
{
	...
	public class AmbientOcclusion : MonoBehaviour
	{
		...
		private void BuildAOCommands()
		{
			...
			commandBuffer.Blit(null, num2, material, 0);
	...
	public class TonemappingColorGrading : MonoBehaviour
	{
		...
		{
			validUserLutSize = IsLutSizeValid(lut.texture);
			return validUserLutSize;
		}

		private bool IsLutSizeValid(Texture texture)
		{
			return texture.height == (int)Mathf.Sqrt(texture.width);
		}
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			if (LutOverrideTexture != null)
			{
				if (IsLutSizeValid(LutOverrideTexture))
				{
					material.SetTexture("_OverrideLutTex", LutOverrideTexture);
					material.SetVector("_OverrideLutParams", new Vector4(1f / (float)LutOverrideTexture.width, 1f / (float)LutOverrideTexture.height, (float)LutOverrideTexture.height - 1f, LutOverrideAmount));
				}
				else
				{
					Debug.LogWarningFormat("Override texture {0} has invalid size to be used as LUT.", LutOverrideTexture.name);
				}
			}
