...
namespace RecRoom.Circuits
{
	...
	public class PlayerRoleMappingNode : PostEvalActionNode
	{
		...
		{
			masterLastPlayerHasRoleSignal = GetPlayerHasRoleSignal(isMasterClient);
			SetContinuousOutput(OutputSlot.G, CircuitNode.BoolToSignal(masterLastPlayerHasRoleSignal));
		}

		private bool GetPlayerHasRoleSignal(bool isMasterClient)
		{
			if (!isMasterClient)
			{
			...
			if (inputSignal != 0)
			{
				try
				{
					return this.PlayerHasRole(inputSignal, ModificationRoleId);
				}
				catch (Exception e)
				{
					CircuitDebug.LogException(e);
