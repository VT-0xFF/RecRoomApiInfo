...
namespace Parabox.CSG
{
	internal class CSG_Model
	{
		...
		{
			vertices = new List<CSG_Vertex>();
			UnityEngine.Mesh sharedMesh = go.GetComponent<MeshFilter>().sharedMesh;
	...
	public class CSG
	{
		...
		public static UnityEngine.Mesh Union(GameObject lhs, GameObject rhs)
		{
		...
		public static UnityEngine.Mesh Subtract(GameObject lhs, GameObject rhs)
		{
			CSG_Model cSG_Model = new CSG_Model(lhs);
			CSG_Model cSG_Model2 = new CSG_Model(rhs);
			CSG_Node a = new CSG_Node(cSG_Model.ToPolygons());
		...
		public static UnityEngine.Mesh Intersect(GameObject lhs, GameObject rhs)
		{
			CSG_Model cSG_Model = new CSG_Model(lhs);
			CSG_Model cSG_Model2 = new CSG_Model(rhs);
			CSG_Node a = new CSG_Node(cSG_Model.ToPolygons());
...
namespace ProBuilder2.MeshOperations
{
	public static class pbMeshOps
	{
		...
		public static bool Extrude(this pb_Object pb, pb_Face[] faces, float extrudeDistance, bool extrudeAsGroup, out pb_Face[] appendedFaces)
		{
			...
			Vector3[] vertices = pb.vertices;
			pb_Edge[][] array = (extrudeAsGroup ? new pb_Edge[1][] { pbMeshUtils.GetPerimeterEdges(pb, dictionary, faces).ToArray() } : faces.Select((pb_Face pb_Face6) => pb_Face6.edges).ToArray());
			if (array == null || array.Length < 1 || (extrudeAsGroup && array[0].Length < 3))
			{
			...
			for (int num2 = 0; num2 < array.Length; num2++)
			{
				...
				{
					array4 = faces;
					foreach (pb_Face pb_Face in array4)
					{
			...
			List<pb_Edge>[] array5 = new List<pb_Edge>[array.Length];
			...
			for (int num5 = 0; num5 < array.Length; num5++)
			{
				...
				for (int num6 = 0; num6 < array[num5].Length; num6++)
				{
					...
					if (Mathf.Abs(extrudeDistance) > float.Epsilon)
					{
						if (!extrudeAsGroup)
						{
							vector = pb_Math.Normal(vertices[pb_Face2.indices[0]], vertices[pb_Face2.indices[1]], vertices[pb_Face2.indices[2]]);
							vector2 = vector;
						}
						else
						{
							vector = Norm(sharedIndices[dictionary[pb_Edge.x]], array3[num5], normals);
							vector2 = Norm(sharedIndices[dictionary[pb_Edge.y]], array3[num5], normals);
						}
					}
					...
					list.Add(new Vector3[4]
					{
						vertices[pb_Edge.x],
						vertices[pb_Edge.y],
						vertices[pb_Edge.x] + vector.normalized * extrudeDistance,
						vertices[pb_Edge.y] + vector2.normalized * extrudeDistance
					});
					...
					array5[num5].Add(new pb_Edge(num7, -1));
					array5[num5].Add(new pb_Edge(num8, -1));
				}
			}
			appendedFaces = pb.AppendFaces(list.ToArray(), list2.ToArray(), list3.ToArray(), list4.ToArray(), list5.ToArray());
			int num9 = 0;
			int num10 = 0;
			for (; num9 < array5.Length; num9++)
			{
				for (int num11 = 0; num11 < array5[num9].Count; num11 += 2)
				{
					array5[num9][num11].y = appendedFaces[num10].indices[2];
					array5[num9][num11 + 1].y = appendedFaces[num10++].indices[4];
				}
			}
			...
			{
				for (int num13 = 0; num13 < array5[num12].Count - 1; num13++)
				{
					int x = array5[num12][num13].x;
					for (int num14 = num13 + 1; num14 < array5[num12].Count; num14++)
					{
						if (array5[num12][num14].x == x)
						{
							dictionary2[array5[num12][num13].y] = dictionary2[array5[num12][num14].y];
							break;
						}
					}
				}
			...
			array4 = faces;
			foreach (pb_Face pb_Face3 in array4)
			{
			...
			{
				array4 = faces;
				foreach (pb_Face pb_Face4 in array4)
				{
					int[] distinctIndices = pb_Face4.distinctIndices;
					int[] array6 = distinctIndices;
					foreach (int num16 in array6)
					{
						int num17 = sharedIndices2.IndexOf(num16);
						for (int num18 = 0; num18 < array3.Length; num18++)
						{
							for (int num19 = 0; num19 < array5[num18].Count; num19++)
							{
								if (num17 == array5[num18][num19].x)
								{
									dictionary2[num16] = dictionary2[array5[num18][num19].y];
									break;
								}
							}
						}
			...
			else
			{
				...
				{
					int[] array6 = pb_Face.AllTrianglesDistinct(array2[num20]);
					foreach (int key in array6)
					{
						int old_si_index = dictionary[key];
						int num21 = array5[num20].FindIndex((pb_Edge pb_Edge2) => pb_Edge2.x == old_si_index);
						if (num21 >= 0)
						{
							int y = array5[num20][num21].y;
			...
			array4 = faces;
			foreach (pb_Face pb_Face5 in array4)
			{
				if (!extrudeAsGroup)
				{
					vector3 = pb_Math.Normal(vertices[pb_Face5.indices[0]], vertices[pb_Face5.indices[1]], vertices[pb_Face5.indices[2]]);
				}
				int[] array6 = pb_Face5.distinctIndices;
				foreach (int num22 in array6)
				{
					if (extrudeAsGroup)
					{
						vector3 = Norm(sharedIndices[dictionary[num22]], all, normals);
					}
					vertices[num22] += vector3.normalized * extrudeDistance;
				}
			}
		...
		public static bool Extrude(this pb_Object pb, pb_Edge[] edges, float extrudeDistance, bool extrudeAsGroup, bool enableManifoldExtrude, out pb_Edge[] extrudedEdges)
		{
			...
			for (int l = 0; l < list.Count; l++)
			{
				...
				Vector3 vector = (extrudeAsGroup ? Norm(sharedIndices[dictionary[pb_Edge2.x]], array, normals) : pb_Math.Normal(pb, pb_Face2));
				Vector3 vector2 = (extrudeAsGroup ? Norm(sharedIndices[dictionary[pb_Edge2.y]], array, normals) : pb_Math.Normal(pb, pb_Face2));
		...
		{
			return pb.Bridge(a, b, enforcePerimiterEdgesOnly: true);
		}
		...
		public static bool CombineObjects(pb_Object[] pbs, out pb_Object combined)
		{
			...
			GameObject gameObject = (GameObject)UnityEngine.Object.Instantiate((UnityEngine.Object)pbs[0].gameObject);
		...
		{
			UnityEngine.Mesh sharedMesh = t.GetComponent<MeshFilter>().sharedMesh;
			Vector3[] vertices = sharedMesh.vertices;
			Color[] array = sharedMesh.colors ?? new Color[vertices.Length];
			Vector2[] uv = sharedMesh.uv;
			...
			for (int i = 0; i < sharedMesh.subMeshCount; i++)
			{
				...
				for (int j = 0; j < triangles.Length; j += 3)
				{
					...
					if (num > -1 && preserveFaces)
					{
						...
						Array.Copy(list4[num].indices, 0, array2, 0, num2);
						array2[num2] = triangles[j];
						array2[num2 + 1] = triangles[j + 1];
						array2[num2 + 2] = triangles[j + 2];
						list4[num].SetIndices(array2);
					...
					{
						list.Add(vertices[triangles[j]]);
						list.Add(vertices[triangles[j + 1]]);
						list.Add(vertices[triangles[j + 2]]);
						list2.Add((array != null) ? array[triangles[j]] : Color.white);
						list2.Add((array != null) ? array[triangles[j + 1]] : Color.white);
						list2.Add((array != null) ? array[triangles[j + 2]] : Color.white);
						list3.Add(uv[triangles[j]]);
						list3.Add(uv[triangles[j + 1]]);
						list3.Add(uv[triangles[j + 2]]);
						i2 = new int[3]
						{
							j,
							j + 1,
					...
					list4.Add(new pb_Face(i2, t.GetComponent<MeshRenderer>().sharedMaterials[i], new pb_UV(), 0, -1, -1, manualUV: true));
				}
			}
			GameObject gameObject = (GameObject)UnityEngine.Object.Instantiate((UnityEngine.Object)t.gameObject);
		...
		public static bool ResetPbObjectWithMeshFilter(pb_Object pb, bool preserveFaces)
		{
			...
			Vector3[] vertices = sharedMesh.vertices;
			Color[] array = ((sharedMesh.colors != null && sharedMesh.colors.Length == vertexCount) ? sharedMesh.colors : new Color[vertexCount]);
			Vector2[] array2 = ((sharedMesh.uv == null || sharedMesh.uv.Length != vertexCount) ? new Vector2[vertexCount] : sharedMesh.uv);
			List<Vector3> list = (preserveFaces ? new List<Vector3>(sharedMesh.vertices) : new List<Vector3>());
			List<Color> list2 = (preserveFaces ? new List<Color>(sharedMesh.colors) : new List<Color>());
			List<Vector2> list3 = (preserveFaces ? new List<Vector2>(sharedMesh.uv) : new List<Vector2>());
			List<pb_Face> list4 = new List<pb_Face>();
			...
			for (int i = 0; i < sharedMesh.subMeshCount; i++)
			{
				...
				for (int j = 0; j < triangles.Length; j += 3)
				{
					...
					if (num2 > -1 && preserveFaces)
					{
						...
						Array.Copy(list4[num2].indices, 0, array3, 0, num3);
						array3[num3] = triangles[j];
						array3[num3 + 1] = triangles[j + 1];
						array3[num3 + 2] = triangles[j + 2];
						list4[num2].SetIndices(array3);
					...
					else
					{
						...
						list.Add(vertices[triangles[j + 1]]);
						list.Add(vertices[triangles[j + 2]]);
						list2.Add((array != null && array.Length == vertexCount) ? array[triangles[j]] : Color.white);
						list2.Add((array != null && array.Length == vertexCount) ? array[triangles[j + 1]] : Color.white);
						list2.Add((array != null && array.Length == vertexCount) ? array[triangles[j + 2]] : Color.white);
						list3.Add(array2[triangles[j]]);
						list3.Add(array2[triangles[j + 1]]);
						list3.Add(array2[triangles[j + 2]]);
						i2 = new int[3]
						{
							j,
							j + 1,
					...
					list4.Add(new pb_Face(i2, sharedMaterials[(i >= num) ? (num - 1) : i], new pb_UV(), 0, -1, -1, manualUV: true));
				}
			}
	...
	public class pbMeshUtils
	{
		...
		{
			List<pb_Face> list = new List<pb_Face>();
			pb_Edge[] array = GetPerimeterEdges(pb, sharedIndicesLookup, selFaces).ToArray();
		...
		private static bool GetEdgeLoopInternal(pb_WingedEdge start, int startIndex, HashSet<pb_EdgeLookup> used)
		{
			...
			do
			{
				...
				if (list != null && list.Count == 4)
				{
					...
					num = ((pb_WingedEdge.edge.common.x == num) ? pb_WingedEdge.edge.common.y : pb_WingedEdge.edge.common.x);
				}
			}
...
namespace ProBuilder2.MeshOperations
{
	public static class pbTriangleOps
	{
		...
		public static bool FlipEdge(this pb_Object pb, pb_Face face)
		{
			...
			int num2 = indices[(array[3] == 1) ? 3 : ((array[4] == 1) ? 4 : 5)];
			int num3 = -1;
			if (array[0] == 2)
			{
				num3 = indices[0];
		...
		public static bool RemoveDegenerateTriangles(this pb_Object pb, out int[] removed)
		{
			...
			foreach (pb_Face pb_Face in faces)
			{
				...
				for (int j = 0; j < indices.Length; j += 3)
				{
					int[] array = new int[3]
					{
						sharedIndices.IndexOf(indices[j]),
						sharedIndices.IndexOf(indices[j + 1]),
						sharedIndices.IndexOf(indices[j + 2])
					};
					float num = pb_Math.TriangleArea(vertices[indices[j]], vertices[indices[j + 1]], vertices[indices[j + 2]]);
					if (array[0] != array[1] && array[0] != array[2] && array[1] != array[2] && !(num <= 0f))
					{
						list3.Add(indices[j]);
						list3.Add(indices[j + 1]);
						list3.Add(indices[j + 2]);
					}
				}
				if (list3.Count > 0)
				{
					pb_Face.SetIndices(list3.ToArray());
	...
	public static class pbVertexOps
	{
		public static bool MergeVertices(this pb_Object pb, int[] indices, out int collapsedIndex, bool collapseToFirst = false)
		{
			...
			pb_Vertex vertex = (collapseToFirst ? vertices[indices[0]] : pb_Vertex.Average(vertices, indices));
			pb_IntArray[] sharedIndices = pb.sharedIndices;
			pb_IntArray[] sharedIndices2 = pb.sharedIndicesUV;
		...
		public static pb_ActionResult WeldVertices(this pb_Object pb, int[] indices, float delta, out int[] welds)
		{
			...
			foreach (int item2 in commonIndices)
			{
				if (!dictionary2.ContainsKey(item2))
				{
					...
					array2[0] = position2.x;
					array2[1] = position2.y;
					array2[2] = position2.z;
					NearestNeighbour<int> nearestNeighbour = kDTree.NearestNeighbors(array2, 64, delta);
					Vector3 zero = Vector3.zero;
					int num2 = 0;
					while (nearestNeighbour.MoveNext())
					{
						int current3 = nearestNeighbour.Current;
						zero += vertices[sharedIndices[current3][0]].position;
						dictionary2.Add(current3, num);
						num2++;
					}
					zero /= (float)num2;
					dictionary3.Add(num, zero);
					num++;
				}
			}
			...
			foreach (KeyValuePair<int, int> item3 in dictionary2)
			{
				...
				{
					dictionary[array3[i]] = item3.Value;
					vertices[array3[i]].position = dictionary3[item3.Value];
				}
			}
		...
		public static bool AppendVerticesToFace(this pb_Object pb, pb_Face face, Vector3[] points, Color[] addColors, out pb_Face newFace)
		{
			...
			List<int> list5 = new List<int>();
			List<int> list6 = ((dictionary2 != null) ? new List<int>() : null);
		...
		{
			List<int> list = new List<int>();
			int[] source = pb_Face.AllTriangles(pb.faces);
		...
		public static void DeleteVerticesWithIndices(this pb_Object pb, IEnumerable<int> distInd)
		{
			...
			for (int i = 0; i < faces.Length; i++)
			{
				...
				faces[i].SetIndices(indices);
			}
			pb_IntArray[] sharedIndices = pb.sharedIndices;
			pb_IntArray[] sharedIndices2 = pb.sharedIndicesUV;
			pb_IntArrayUtility.RemoveValuesAndShift(ref sharedIndices, distInd);
			pb_IntArrayUtility.RemoveValuesAndShift(ref sharedIndices2, distInd);
			pb.SetSharedIndices(sharedIndices);
			pb.SetSharedIndicesUV(sharedIndices2);
			pb.SetVertices(vertices);
			pb.SetFaces(faces);
		...
		public static void Quantize(pb_Object pb, IList<int> indices, Vector3 snap)
		{
			...
			for (int i = 0; i < indices.Count; i++)
			{
				...
				reference = pb.transform.InverseTransformPoint(pbUtil.SnapValue(pb.transform.TransformPoint(vertices[indices[i]]), snap));
			}
		}
	}
	...
	public static class pb_AppendPolygon
	{
		public static pb_ActionResult CreatePolygon(this pb_Object pb, IList<int> indices, bool unordered, out pb_Face face)
		{
			...
			face = null;
			return new pb_ActionResult(Status.Failure, unordered ? "Too Few Unique Points Selected" : "Points not ordered correctly");
		}
		...
		public static List<List<pb_WingedEdge>> FindHoles(List<pb_WingedEdge> wings, HashSet<int> common)
		{
			...
			for (int i = 0; i < wings.Count; i++)
			{
				...
				while (pb_WingedEdge2 != null)
				{
					...
					num = ((pb_WingedEdge2.edge.common.x == num) ? pb_WingedEdge2.edge.common.y : pb_WingedEdge2.edge.common.x);
				...
				for (int j = 0; j < list2.Count; j++)
				{
					...
					for (int num2 = j - 1; num2 > -1; num2--)
					{
						if (pb_WingedEdge3.edge.common.y == list2[num2].edge.common.x)
						{
							list3.Add(new pb_Tuple<int, int>(num2, j));
							break;
						}
					}
				}
				...
				{
					int item = list3[num3].Item1;
					int num4 = list3[num3].Item2 - array[num3];
					int num5 = num4 - item + 1;
					List<pb_WingedEdge> range = list2.GetRange(item, num5);
					list2.RemoveRange(item, num5);
					for (int num6 = num3 - 1; num6 > -1; num6--)
					{
						if (list3[num6].Item2 > list3[num3].Item2)
						{
							array[num6] += num5;
						}
					}
		...
		private static pb_WingedEdge FindNextEdgeInHole(pb_WingedEdge wing, int common)
		{
			...
			while (adjacentEdgeWithCommonIndex != null && adjacentEdgeWithCommonIndex != wing)
			{
	...
	public static class pb_Bevel
	{
		...
		public static pb_ActionResult BevelEdges(pb_Object pb, IList<pb_Edge> edges, float amount, out List<pb_Face> createdFaces)
		{
			...
			{
				Func<pb_WingedEdge, bool> predicate = (pb_WingedEdge x) => x.edge.Equals(lup);
				pb_WingedEdge pb_WingedEdge = wingedEdges.FirstOrDefault(predicate);
				num++;
			...
			{
				Func<pb_WingedEdge, bool> predicate2 = (pb_WingedEdge x) => x.edge.common.Contains(c) && (!ignore.ContainsKey(x.face) || !ignore[x.face].Contains(c));
				IEnumerable<pb_WingedEdge> enumerable = wingedEdges.Where(predicate2);
	...
	public static class pb_ConformNormals
	{
		public static pb_ActionResult ConformNormals(this pb_Object pb, IList<pb_Face> faces)
		{
			...
			if (num > 0)
			{
				return new pb_ActionResult(Status.Success, (num > 1) ? $"Flipped {num} faces" : "Flipped 1 face");
			}
	...
	public static class pb_ConnectVertices
	{
		public static pb_ActionResult Connect(this pb_Object pb, IList<int> indices, out int[] newVertices)
		{
			...
			foreach (KeyValuePair<pb_Face, List<int>> item2 in dictionary)
			{
				...
				List<ConnectFaceRebuildData> list3 = ((item2.Value.Count == 2) ? ConnectIndicesInFace(key, item2.Value[0], item2.Value[1], vertices, lookup) : ConnectIndicesInFace(key, item2.Value, vertices, lookup, num++));
				if (list3 == null)
				{
					continue;
				}
		...
		private static void Flood(pb_Object pb, pb_WingedEdge wing, Vector3 wingNrm, float maxAngle, HashSet<pb_Face> selection)
		{
			...
			do
			{
				...
				if (opposite != null && !selection.Contains(opposite.face))
				{
					...
					{
						Flood(pb, opposite, Vector3.zero, maxAngle, selection);
					}
				}
				pb_WingedEdge = pb_WingedEdge.next;
			}
		...
		public static HashSet<pb_Face> FloodSelection(pb_Object pb, IList<pb_Face> faces, float maxAngleDiff)
		{
			...
			for (int i = 0; i < wingedEdges.Count; i++)
			{
				if (!hashSet2.Contains(wingedEdges[i].face) && hashSet.Contains(wingedEdges[i].face))
				{
					...
					Flood(pb, wingedEdges[i], (maxAngleDiff > 0f) ? pb_Math.Normal(pb, wingedEdges[i].face) : Vector3.zero, maxAngleDiff, hashSet2);
				}
			}
			return hashSet2;
		}
...
namespace ProBuilder2.Common
{
	public static class pb_Object_Utility
	{
		...
		{
			int num = 0;
			int[] array = ((lookup != null) ? pb.sharedIndices.AllIndicesWithValues(lookup, selectedTriangles).ToArray() : pb.sharedIndices.AllIndicesWithValues(selectedTriangles).ToArray());
			Matrix4x4 worldToLocalMatrix = pb.transform.worldToLocalMatrix;
			Vector3 vector = worldToLocalMatrix * offset;
			Vector3[] vertices = pb.vertices;
			if (Mathf.Abs(snapValue) > float.Epsilon)
			{
				Matrix4x4 localToWorldMatrix = pb.transform.localToWorldMatrix;
				Vector3 zero = Vector3.zero;
				Vector3 vector2 = (snapAxisOnly ? offset.ToMask(0.0001f) : Vector3.one);
				for (num = 0; num < array.Length; num++)
				{
					zero = localToWorldMatrix.MultiplyPoint3x4(vertices[array[num]] + vector);
					ref Vector3 reference = ref vertices[array[num]];
					reference = worldToLocalMatrix.MultiplyPoint3x4(pbUtil.SnapValue(zero, snapValue * vector2));
				}
			}
			else
			{
	...
	public class pb_ShapeGenerator
	{
		...
		{
			bool flag = innerRadius < float.Epsilon;
		...
		public static pb_Object CylinderGenerator(int axisDivisions, float radius, float height, int heightCuts)
		{
			...
			for (int l = 0; l < heightCuts + 1; l++)
			{
				for (int m = 0; m < axisDivisions * 4; m += 4)
				{
					...
					int num8 = num5 + 2;
					int num9 = num5 + 3;
					array3[num4++] = new pb_Face(new int[6] { num6, num7, num8, num7, num9, num8 });
				}
			}
		...
		public static pb_Object DoorGenerator(float totalWidth, float totalHeight, float ledgeHeight, float legWidth, float depth)
		{
			...
			Array.Resize(ref array2, 88);
			for (int i = 30; i < 60; i++)
			{
				ref Vector3 reference = ref array2[i];
				reference = array2[i - 30];
				array2[i].z = 0f - array2[i].z;
			}
			ref Vector3 reference2 = ref array2[60];
			reference2 = array[1];
			ref Vector3 reference3 = ref array2[61];
			reference3 = new Vector3(array[1].x, array[1].y, 0f - array[1].z);
			ref Vector3 reference4 = ref array2[62];
			reference4 = array[5];
			ref Vector3 reference5 = ref array2[63];
			reference5 = new Vector3(array[5].x, array[5].y, 0f - array[5].z);
			ref Vector3 reference6 = ref array2[64];
			reference6 = array[5];
			ref Vector3 reference7 = ref array2[65];
			reference7 = new Vector3(array[5].x, array[5].y, 0f - array[5].z);
			ref Vector3 reference8 = ref array2[66];
			reference8 = array[6];
			ref Vector3 reference9 = ref array2[67];
			reference9 = new Vector3(array[6].x, array[6].y, 0f - array[6].z);
			ref Vector3 reference10 = ref array2[68];
			reference10 = array[6];
			ref Vector3 reference11 = ref array2[69];
			reference11 = new Vector3(array[6].x, array[6].y, 0f - array[6].z);
			ref Vector3 reference12 = ref array2[70];
			reference12 = array[2];
			ref Vector3 reference13 = ref array2[71];
			reference13 = new Vector3(array[2].x, array[2].y, 0f - array[2].z);
			...
			Array.Resize(ref array3, 78);
			for (int j = 30; j < 60; j += 3)
			{
				array3[j + 2] = array3[j - 30] + 30;
				array3[j + 1] = array3[j - 29] + 30;
				array3[j] = array3[j - 28] + 30;
			}
			int num2 = 60;
			for (int k = 60; k < 78; k += 6)
			{
				array3[k] = num2;
				array3[k + 1] = num2 + 1;
				array3[k + 2] = num2 + 2;
			...
			pb_Face[] array4 = new pb_Face[13];
			for (int l = 0; l < 13; l++)
			{
				int[] array5 = new int[6];
				Array.Copy(array3, l * 6, array5, 0, 6);
				array4[l] = new pb_Face(array5);
			}
			pb_Object pb_Object = pb_Object.CreateInstanceWithVerticesFaces(array2, array4);
		...
		public static pb_Object PlaneGenerator(float _width, float _height, int widthCuts, int heightCuts, Axis axis, bool smooth)
		{
			...
			Vector2[] array = (smooth ? new Vector2[num * num2] : new Vector2[num * num2 * 4]);
			Vector3[] array2 = (smooth ? new Vector3[num * num2] : new Vector3[num * num2 * 4]);
		...
		private static Vector3[] CircleVertices(int segments, float radius, float circumference, Quaternion rotation, float offset)
		{
			...
			ref Vector3 reference = ref array[0];
			reference = new Vector3(Mathf.Cos(0f / num * circumference * ((float)System.Math.PI / 180f)) * radius, Mathf.Sin(0f / num * circumference * ((float)System.Math.PI / 180f)) * radius, 0f);
			ref Vector3 reference2 = ref array[1];
			reference2 = new Vector3(Mathf.Cos(1f / num * circumference * ((float)System.Math.PI / 180f)) * radius, Mathf.Sin(1f / num * circumference * ((float)System.Math.PI / 180f)) * radius, 0f);
...
namespace ProBuilder2.MeshOperations
{
	...
	{
		private static TriangleNet.Mesh _tmesh;

		private static TriangleNet.Mesh GetTMesh(bool convex)
		{
			if (_tmesh == null)
			{
				Behavior behavior = new Behavior();
				behavior.Convex = convex;
				behavior.ConformingDelaunay = false;
				behavior.NoBisect = 2;
				behavior.NoHoles = true;
				behavior.Jettison = false;
				_tmesh = new TriangleNet.Mesh(behavior);
			}
		...
		public static bool TriangulateVertices(IList<pb_Vertex> vertices, out List<int> triangles, bool unordered = true, bool convex = false)
		{
			...
			Vector3 normal = pb_Projection.FindBestPlane(array).normal;
			Vector2[] points = pb_Projection.PlanarProject(array, normal);
			if (unordered)
			{
		...
		public static bool Triangulate(IList<Vector2> points, out List<int> indices, bool convex = false)
		{
			int count = points.Count;
			indices = new List<int>();
			InputGeometry inputGeometry = new InputGeometry(count);
			WindingOrder windingOrder = pbTriangleOps.GetWindingOrder(points);
			for (int i = 0; i < count; i++)
			{
				inputGeometry.AddPoint(points[i].x, points[i].y, 2);
				inputGeometry.AddSegment(i, (i + 1) % count, 2);
			}
			TriangleNet.Mesh tMesh = GetTMesh(convex);
			tMesh.Triangulate(inputGeometry);
			if (tMesh.Vertices.Count != points.Count)
			{
			...
			tMesh.Renumber(NodeNumbering.Linear);
