...
namespace RecRoom.Circuits
{
	...
	public abstract class CircuitNode
	{
		...
		public virtual CircuitNodeData Serialize()
		{
			CircuitNodeData circuitNodeData = new CircuitNodeData
			{
				...
				NodeType = Serialization.GetNodeType(this).ToByteString()
			};
		...
		public virtual void Init(CircuitNodeData data, bool reassignNodeIds, Dictionary<int, int> nodeIdMap)
		{
			...
			{
				int id = nodeIdMap[data.PairedNodeId];
				PairedNode = Find(id);
			}
		}
		...
		internal static bool ConnectNodes(CircuitNode sourceNode, CircuitNode destNode, InputSlot inputSlot = InputSlot.R, OutputSlot outputSlot = OutputSlot.R, bool skipMerge = false)
		{
			if (sourceNode == null)
			{
				throw new InvalidOperationException("SourceNode cannot be null");
			}
			if (destNode == null)
			{
				throw new InvalidOperationException("DestNode cannot be null");
			}
		...
		protected virtual void _EvaluateNode()
		{
		}

		protected virtual void _ResetNode()
		{
			ClearOutputs();
		}
	...
	public delegate void UpdateGraph(int networkTimeMilliseconds);
	public class CircuitNodeGraph : IComparable<CircuitNodeGraph>
	{
		...
		public static void Update(int networkTimeMilliseconds)
		{
			if (CircuitNodeGraph.TimeUpdate != null)
			{
				CircuitNodeGraph.TimeUpdate(networkTimeMilliseconds);
			}
		}
		...
		public List<CircuitNode> Evaluate()
		{
			...
			for (int num = ReverseTopologicNodes.Count - 1; num >= 0; num--)
			{
				...
				ReverseTopologicNodes[num].EvaluateNode();
		...
		public static CircuitNodeGraph Deserialize(CircuitNodeGraphData data, bool reassignNodeIds, out Dictionary<int, int> nodeIdMap)
		{
			...
			if (circuitNodeGraph.RootNode == null)
			{
				throw new InvalidOperationException("No root node for graph found!");
			}
		...
		{
			if (nodeA.NodeGraph == null || nodeB.NodeGraph == null)
			{
				throw new InvalidOperationException("NodeGraphs should never be null!");
			}
		...
		public void Visualize(string fileName, bool useDFS = false)
		{
			Visualize("C:\\RecRoom\\Circuits\\CircuitsApp\\HTML.txt", fileName, useDFS);
		}

		public void VisualizeWithFormatString(string htmlString, string outputFileName, bool useDFS = false)
		{
		...
		public void Visualize(string formatStringFilePath, string outputFileName, bool useDFS = false)
		{
			string htmlString = File.ReadAllText(formatStringFilePath);
			VisualizeWithFormatString(htmlString, outputFileName, useDFS);
		}
	...
	public class BooleanNode : CircuitNode
	{
		...
		protected override void _EvaluateNode()
		{
			int val = CalcOutput();
		...
		private void FetchInputs()
		{
			...
			for (int i = 0; i < inputMetadata.Length; i++)
			{
				...
				if (Inputs.ContainsKey(inputMetadata2.InputSlot))
				{
	...
	public class ContinuousSourceNode : SourceNode
	{
		...
		public override void SetOutput(int value)
		{
			SetContinuousOutput(OutputSlot.R, value);
		}

		public override void SetOutput(int value1, int value2)
		{
			SetContinuousOutput(OutputSlot.R, value1);
			SetContinuousOutput(OutputSlot.G, value2);
		}

		public override void SetOutput(int value1, int value2, int value3)
		{
			SetContinuousOutput(OutputSlot.R, value1);
			SetContinuousOutput(OutputSlot.G, value2);
			SetContinuousOutput(OutputSlot.B, value3);
		}
	}
	public abstract class SourceNode : CircuitNode
	{
		public enum OutputCounts
		{
			One = 1,
			Two,
			Three
		...
		private OutputCounts _outputCount = OutputCounts.One;

		public OutputCounts OutputCount
		{
			get
			{
				return _outputCount;
		...
		protected override OutputMetadata[] GetOutputMetadata()
		{
			if (OutputMeta.Length != (int)OutputCount)
			{
				RefreshOutputCount();
			}
			return OutputMeta;
		...
		public abstract void SetOutput(int value);

		public abstract void SetOutput(int value1, int value2);

		public abstract void SetOutput(int value1, int value2, int value3);

		private void RefreshOutputCount()
		{
			int outputCount = (int)OutputCount;
			if (OutputMeta.Length != outputCount)
			{
				switch (outputCount)
				{
				case 1:
					OutputMeta = new OutputMetadata[1]
					{
						new OutputMetadata
						{
							OutputSlot = OutputSlot.R
					...
					break;
				case 2:
					OutputMeta = new OutputMetadata[2]
					{
						new OutputMetadata
						{
							OutputSlot = OutputSlot.R
					...
					break;
				case 3:
					OutputMeta = new OutputMetadata[3]
					{
						new OutputMetadata
						{
							OutputSlot = OutputSlot.R
					...
					break;
				default:
					throw new ArgumentException("SourceNode can only accept 1, 2, or 3 outputs");
				}
			}
		}
	}
	...
	public class DelayNode : CircuitNode
	{
		...
		protected override void _ResetNode()
		{
			base._ResetNode();
			QueuedSignals.Clear();
		}
		...
		private void OnTimerUpdate(int networkTime)
		{
	...
	public class MomentarySourceNode : SourceNode
	{
		...
		public override void SetOutput(int value)
		{
			SetMomentaryOutput(OutputSlot.R, value);
		}

		public override void SetOutput(int value1, int value2)
		{
			SetMomentaryOutput(OutputSlot.R, value1);
			SetMomentaryOutput(OutputSlot.G, value2);
		}
		...
		public override void SetOutput(int value1, int value2, int value3)
		{
			SetMomentaryOutput(OutputSlot.R, value1);
			SetMomentaryOutput(OutputSlot.G, value2);
			SetMomentaryOutput(OutputSlot.B, value3);
		}
	}
	[CircuitNodeUuid("6402C773-5838-4495-BC54-BC36EDE4AB0F")]
	public class ConstantSourceNode : SourceNode
	{
		internal override bool IsStateful => true;
	...
	public class OutputNode : CircuitNode
	{
		...
		public delegate void OutputNodeUpdated(int signal0, int signal1, int signal2);
		...
		private InputCounts _inputCount = InputCounts.One;

		public OutputNodeUpdated OutputNodeUpdatedEvent;

		private int[] lastSignals;

		public InputCounts InputCount
		{
			get
			{
				return _inputCount;
		...
		public int OutputSignal0 => GetInputSignal(InputSlot.R);

		public int OutputSignal1 => GetInputSignal(InputSlot.G);

		public int OutputSignal2 => GetInputSignal(InputSlot.B);
		...
		protected override InputMetadata[] GetInputMetadata()
		{
			if (InputMeta.Length != (int)InputCount)
			{
				RefreshInputCount();
			}
			return InputMeta;
		...
		protected override void _EvaluateNode()
		{
			...
			{
				int inputSignal = GetInputSignal((InputSlot)i);
				if (inputSignal != lastSignals[i])
				{
					flag = true;
					lastSignals[i] = inputSignal;
				}
			}
			if (flag && OutputNodeUpdatedEvent != null)
			{
				OutputNodeUpdatedEvent(lastSignals[0], lastSignals[1], lastSignals[2]);
			}
		}
		...
		private void RefreshInputCount()
		{
			...
			if (OutputMeta.Length != inputCount)
			{
				switch (inputCount)
				{
				case 1:
					InputMeta = new InputMetadata[1]
					{
						new InputMetadata
						{
							InputSlot = InputSlot.R
				...
				case 3:
					...
					break;
				default:
					throw new ArgumentException("OutputNode can only accept 1, 2, or 3 inputs");
				}
				lastSignals = new int[3];
			}
		}
	}
	...
	public static class Serialization
	{
		...
		internal static Guid GetNodeType(CircuitNode node)
		{
	...
	internal static class SerializationLookup
	{
		internal static Dictionary<Type, Guid> TypeToGuidLookup = new Dictionary<Type, Guid>
		{
			{
				typeof(BooleanNode),
				new Guid("2385e75e-e0da-45bf-8d66-17dc9ffb9fc7")
			},
			{
				typeof(ContinuousSourceNode),
				new Guid("b7ba1a4b-ea04-496f-b8b6-a2fb7c4a42b9")
			},
			{
				typeof(ComparerNode),
				new Guid("92bd57fa-d68e-4a6e-9ecf-d82985169b82")
			},
			{
				typeof(CombinatorNode),
				new Guid("17b72de0-4116-41db-a99d-1523625f7ef1")
			},
			{
				typeof(DelayNode),
				new Guid("34452a12-afb3-4a4d-97b0-9d25f7e777c5")
			},
			{
				typeof(LoopNode),
				new Guid("c9a59f26-0a72-4c2e-b239-6a83998e0839")
			},
			{
				typeof(MomentarySourceNode),
				new Guid("e7efd8ca-92fe-4a37-9e7c-abbe9e4de336")
			},
			{
				typeof(ConstantSourceNode),
				new Guid("6402c773-5838-4495-bc54-bc36ede4ab0f")
			},
			{
				typeof(NodeGraphNode),
				new Guid("738b2af2-eebd-4316-82a9-2fbec9b50d8b")
			},
			{
				typeof(OutputNode),
				new Guid("c6ab70a0-c752-466a-8070-029742655556")
			},
			{
				typeof(TimerNode),
				new Guid("a77191f7-792b-4590-aab3-a3b71c440dd1")
			},
			{
				typeof(RootNode),
				new Guid("a4160690-4c68-40c2-bd51-b665f71fcebd")
			},
			{
				typeof(NodeGraphNode.GraphInputNode),
				new Guid("86b6fd1c-4668-4ba9-8acb-fb91e25dbfb6")
			},
			{
				typeof(NodeGraphNode.GraphOutputNode),
				new Guid("f8a998db-0354-4bcd-b0e4-6bba7cc24b89")
			}
		};
