...
namespace UnityStandardAssets.CinematicEffects
{
	...
	public class AmbientOcclusion : MonoBehaviour
	{
		private struct PropertyObserver
		{
			...
			{
				return _blurIterations != setting.blurIterations || _downsampling != setting.downsampling || _ambientOnly != setting.ambientOnly || _pixelWidth != camera.pixelWidth || _pixelHeight != camera.pixelHeight;
			}
		...
		public bool isAmbientOnlySupported => targetCamera.hdr && isGBufferAvailable;
		...
		private int blurIterations => settings.blurIterations;
		...
		private bool isGBufferAvailable
		{
			get
			{
				RenderingPath actualRenderingPath = targetCamera.actualRenderingPath;
		...
		private void BuildAOCommands()
		{
			...
			Texture source = null;
			commandBuffer.Blit(source, num3, material, 0);
			if (blurIterations > 0)
			{
	...
	public class Bloom : MonoBehaviour
	{
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			material.SetFloat("_PrefilterOffs", (!flag2) ? 0f : (-0.5f));
	...
	public class LensAberrations : MonoBehaviour
	{
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			if (distortion.enabled)
			{
				...
				Vector4 vector = new Vector4(2f * distortion.centerX - 1f, 2f * distortion.centerY - 1f, distortion.amountX, distortion.amountY);
			...
			if (chromaticAberration.enabled && !Mathf.Approximately(chromaticAberration.tangential, 0f))
			{
				...
				Vector4 vector3 = new Vector4(2.5f * chromaticAberration.tangential, 5f * chromaticAberration.axial, 5f / Mathf.Max(Mathf.Epsilon, chromaticAberration.contrastDependency), 5f);
