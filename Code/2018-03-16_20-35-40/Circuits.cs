...
namespace RecRoom.Circuits
{
	...
	public class QueuedMomentarySourceNode : SourceNode
	{
		...
		{
			Console.Out.WriteLine(base.NodeId + " Queueing output: " + outputs.Select((Output _) => _.outputSlot.ToString() + " " + _.outputValue).Aggregate((string s, string s1) => s + "," + s1));
	...
	public class ComparerNode : CircuitNode
	{
		...
		{
			bool num = CalcOutput();
			int val = CircuitNode.BoolToSignal(num);
			SetContinuousOutput(OutputSlot.R, val);
			int val2 = CircuitNode.BoolToSignal(!num);
		...
		protected override OutputMetadata[] GetOutputMetadata()
		{
			...
			return new OutputMetadata[3]
			{
				...
				new OutputMetadata
				{
					...
					Description = "Else " + nodeDescription
				},
				new OutputMetadata
				{
					OutputSlot = OutputSlot.RESET,
