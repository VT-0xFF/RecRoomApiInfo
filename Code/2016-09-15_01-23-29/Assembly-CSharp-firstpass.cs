	...
	public class TonemappingColorGrading : MonoBehaviour
	{
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			if (eyeAdaptation.enabled)
			{
				...
				num3 |= num3 >> 16;
				num3 -= num3 >> 1;
				renderTexture = RenderTexture.GetTemporary(num3, num3, 0, m_AdaptiveRtFormat);
				Graphics.Blit(source, renderTexture);
				int num4 = (int)Mathf.Log(renderTexture.width, 2f);
				int num5 = 2;
				array = new RenderTexture[num4];
				for (int i = 0; i < num4; i++)
				{
					array[i] = RenderTexture.GetTemporary(renderTexture.width / num5, renderTexture.width / num5, 0, m_AdaptiveRtFormat);
					num5 <<= 1;
				}
				RenderTexture source2 = array[num4 - 1];
			...
			if (tonemapping.enabled)
			{
				if (tonemapping.tonemapper == Tonemapper.Curve)
				{
					if (m_TonemapperDirty)
					{
						...
						{
							num7 = tonemapping.curve[tonemapping.curve.length - 1].time;
							for (float num8 = 0f; num8 <= 1f; num8 += 0.003921569f)
							{
								float num9 = tonemapping.curve.Evaluate(num8 * num7);
								tonemapperCurve.SetPixel(Mathf.FloorToInt(num8 * 255f), 0, new Color(num9, num9, num9));
							}
							tonemapperCurve.Apply();
						}
						m_TonemapperCurveRange = 1f / num7;
				...
				{
					float num10 = tonemapping.neutralBlackIn * 20f + 1f;
					float num11 = tonemapping.neutralBlackOut * 10f + 1f;
					float num12 = tonemapping.neutralWhiteIn / 20f;
					float num13 = 1f - tonemapping.neutralWhiteOut / 20f;
					float t = num10 / num11;
					float t2 = num12 / num13;
				...
				num6 = (int)(num6 + (tonemapping.tonemapper + 1));
			}
			if (lut.enabled)
			{
			...
			if (num != 1f)
			{
				material.EnableKeyword("ENABLE_DESATURATION");
				material.SetFloat("_Saturation", num);
			}
			Graphics.Blit(source, destination, material, num6);
			if (eyeAdaptation.enabled)
			{
