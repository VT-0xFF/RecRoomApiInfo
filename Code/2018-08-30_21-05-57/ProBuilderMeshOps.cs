...
namespace ProBuilder.MeshOperations
{
	...
	internal static class pb_Bevel
	{
		...
		public static pb_ActionResult BevelEdges(pb_Object pb, IList<pb_Edge> edges, float amount, out List<pb_Face> createdFaces)
		{
			...
			foreach (KeyValuePair<pb_Face, List<pb_Tuple<pb_WingedEdge, int>>> item5 in dictionary2)
			{
				...
				pb_FaceRebuildData pb_FaceRebuildData = pb_VertexOps.ExplodeVertex(list, item5.Value, amount, out appendedVertices);
				if (pb_FaceRebuildData == null)
				{
					continue;
				}
	...
	public static class pb_ConnectEdges
	{
		...
		private static pb_ActionResult Connect(this pb_Object pb, IEnumerable<pb_Edge> edges, out pb_Face[] addedFaces, out pb_Edge[] connections, bool returnFaces = false, bool returnEdges = false, HashSet<pb_Face> faceMask = null)
		{
			...
			return new pb_ActionResult(Status.Success, $"Connected {list.Count / 2} Edges");
		}
	...
	public static class pb_Extrude
	{
		...
		public static bool Extrude(this pb_Object pb, pb_Edge[] edges, float extrudeDistance, bool extrudeAsGroup, bool enableManifoldExtrude, out pb_Edge[] extrudedEdges)
		{
			...
			for (int l = 0; l < list.Count; l++)
			{
				...
				Vector3 vector = ((!extrudeAsGroup) ? pb_Math.Normal(pb, pb_Face2) : pb_MeshOps.AverageNormalWithIndices(sharedIndices[dictionary[pb_Edge2.x]], array, normals));
				Vector3 vector2 = ((!extrudeAsGroup) ? pb_Math.Normal(pb, pb_Face2) : pb_MeshOps.AverageNormalWithIndices(sharedIndices[dictionary[pb_Edge2.y]], array, normals));
	...
	public static class pb_FaceLoop
	{
		...
		private static HashSet<pb_Face> GetFaceLoop(List<pb_WingedEdge> wings, pb_Face face, bool ring)
		{
			...
			pb_WingedEdge pb_WingedEdge = wings.FirstOrDefault((pb_WingedEdge x) => x.face == face);
			...
			if (ring)
			{
				pb_WingedEdge = pb_WingedEdge.next ?? pb_WingedEdge.previous;
			}
			for (int num = 0; num < 2; num++)
			{
				...
				if (num == 1)
				{
					if (pb_WingedEdge.opposite == null || pb_WingedEdge.opposite.face == null)
					{
						break;
					}
					pb_WingedEdge2 = pb_WingedEdge.opposite;
				}
				while (hashSet.Add(pb_WingedEdge2.face) && pb_WingedEdge2.Count() == 4)
				{
					pb_WingedEdge2 = pb_WingedEdge2.next.next.opposite;
					if (pb_WingedEdge2 == null || pb_WingedEdge2.face == null)
					{
						break;
					}
				}
	...
	internal static class pb_MergeFaces
	{
		...
		public static pb_Face Merge(pb_Object target, IEnumerable<pb_Face> faces)
		{
			...
			pb_Face pb_Face = faces.First();
			pb_Face pb_Face2 = new pb_Face(faces.SelectMany((pb_Face x) => x.indices).ToArray(), pb_Face.material, pb_Face.uv, pb_Face.smoothingGroup, pb_Face.textureGroup, pb_Face.elementGroup, pb_Face.manualUV);
			...
			CollapseCoincidentVertices(target, new pb_Face[1] { pb_Face2 });
			return pb_Face2;
		}
	...
	public class pb_MeshImporter
	{
		...
		public bool Import(Mesh originalMesh, Material[] materials, Settings importSettings = null)
		{
				...
				case MeshTopology.Triangles:
				{
					...
					for (int k = 0; k < indices2.Length; k += 3)
					{
						list2.Add(new pb_Face(new int[3]
						{
							...
							num + 1,
							num + 2
						}, m, new pb_UV(), 0, -1, -1, manualUV: true));
			...
			{
				List<pb_WingedEdge> wingedEdges = pb_WingedEdge.GetWingedEdges(m_Mesh, m_Mesh.faces, oneWingPerFace: true);
				...
				foreach (pb_WingedEdge item2 in wingedEdges)
				{
					...
					{
						if ((item3.opposite == null || !hashSet.Contains(item3.opposite.face)) && dictionary.TryGetValue(item3.edge, out var value) && value > num3 && item2.face == GetBestQuadConnection(item3.opposite, dictionary))
						{
							num3 = value;
							pb_Face = item3.opposite.face;
						}
					}
					if (pb_Face != null)
					{
						hashSet.Add(pb_Face);
						list3.Add(new pb_Tuple<pb_Face, pb_Face>(item2.face, pb_Face));
					}
				}
	...
	internal static class pb_MeshOps
	{
		...
		public static bool Extrude(this pb_Object pb, pb_Face[] faces, ExtrudeMethod method, float extrudeDistance, out pb_Face[] appendedFaces)
		{
			...
			pb_Edge[][] array = ((!flag) ? faces.Select((pb_Face pb_Face6) => pb_Face6.edges).ToArray() : new pb_Edge[1][] { pb_MeshUtils.GetPerimeterEdges(dictionary, faces).ToArray() });
			...
			for (int num5 = 0; num5 < array.Length; num5++)
			{
				...
				for (int num6 = 0; num6 < array[num5].Length; num6++)
				{
					...
					if (Mathf.Abs(extrudeDistance) > Mathf.Epsilon)
					{
						...
						{
							to = AverageNormalWithIndices(sharedIndices[dictionary[pb_Edge.x]], array3[num5], normals);
							to2 = AverageNormalWithIndices(sharedIndices[dictionary[pb_Edge.y]], array3[num5], normals);
						}
					}
			...
			else
			{
				...
				{
					foreach (int item in array2[num24].SelectMany((pb_Face pb_Face6) => pb_Face6.distinctIndices))
					{
						int old_si_index = dictionary[item];
						int num25 = array4[num24].FindIndex((pb_Edge pb_Edge2) => pb_Edge2.x == old_si_index);
						if (num25 >= 0)
						{
			...
			int[] all = faces.SelectMany((pb_Face pb_Face6) => pb_Face6.distinctIndices).ToArray();
	...
	internal static class pb_MeshUtils
	{
		public static List<pb_Face> GetNeighborFaces(pb_Object pb, pb_Face originFace, Dictionary<int, int> lookup = null, IEnumerable<pb_Face> mask = null)
		{
			...
			for (int j = 0; j < pb.faces.Length; j++)
			{
				...
				{
					pb_Edge pb_Edge = edges[k];
					item.x = lookup[pb_Edge.x];
					item.y = lookup[pb_Edge.y];
		...
		public static pb_Face[] GetNeighborFaces(pb_Object pb, Dictionary<int, int> sharedIndicesLookup, pb_Face[] selFaces)
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				ref pb_Edge reference = ref array2[i];
				reference = new pb_Edge(sharedIndicesLookup[array[i].x], sharedIndicesLookup[array[i].y]);
			}
			...
			foreach (pb_Face pb_Face in faces)
			{
				...
				{
					pb_Edge pb_Edge = edges[k];
					value.x = sharedIndicesLookup[pb_Edge.x];
					value.y = sharedIndicesLookup[pb_Edge.y];
	...
	internal static class pb_UVOps
	{
		...
		public static void ProjectFacesAuto(pb_Object pb, pb_Face[] faces)
		{
			...
			for (int num2 = 0; num2 < array.Length; num2++)
			{
				ref Vector2 reference = ref uv[array[num2]];
		...
		public static void ProjectFacesBox(pb_Object pb, pb_Face[] faces)
		{
			...
			foreach (KeyValuePair<ProjectionAxis, List<pb_Face>> item in dictionary)
			{
				...
				{
					ref Vector2 reference = ref uv[array[num]];
		...
		public static pb_ActionResult AppendVerticesToEdge(this pb_Object pb, IList<pb_Edge> edges, int count, out List<pb_Edge> newEdges)
		{
			...
			for (int l = 0; l < list5.Count; l++)
			{
				...
				for (int num5 = 0; num5 < edges2.Length; num5++)
				{
					...
					pb_EdgeLookup pb_EdgeLookup = new pb_EdgeLookup(new pb_Edge(dictionary[local.x], dictionary[local.y]), local);
