...
namespace Google.Protobuf.Collections
{
	...
	public sealed class RepeatedField<T> : IList<T>, ICollection<T>, IEnumerable<T>, IEnumerable, IList, ICollection, IDeepCloneable<RepeatedField<T>>, IEquatable<RepeatedField<T>>
	{
		...
		public void AddRange(T[] source, int srcIndex, int srcLength)
		{
			ProtoPreconditions.CheckNotNull(source, "source");
			if (default(T) == null)
			{
				for (int i = 0; i < source.Length; i++)
				{
					if (source[i] == null)
					{
						throw new ArgumentException("Sequence contained null element", "source");
					}
				}
			}
			EnsureSize(count + srcLength);
			Array.Copy(source, srcIndex, array, count, srcLength);
			count += srcLength;
		}
