...
namespace RecRoom.Circuits
{
	...
	public class ObjectPlayerMappingNode : CircuitNode
	{
		...
		private int lastMasterLastHeldPlayerId;

		private int lastMasterCurrentHoldingPlayerId;
		...
		protected override void _EvaluateNode(bool isMasterClient)
		{
			...
			if (CircuitNode.SignalToBool(inputSignal))
			{
				CalcOutput(inputSignal, isMasterClient);
				SetContinuousOutput(OutputSlot.R, lastMasterLastHeldPlayerId);
				SetContinuousOutput(OutputSlot.G, lastMasterCurrentHoldingPlayerId);
			}
		}

		private void CalcOutput(int objectId, bool isMasterClient)
		{
			if (isMasterClient)
			{
				int lastHeldPlayer = 0;
				int currentHoldingPlayer = 0;
				try
				{
					this.ObjectToHoldingPlayer(objectId, out lastHeldPlayer, out currentHoldingPlayer);
					lastHeldPlayer = CircuitNode.ClampSignal(lastHeldPlayer, 0, int.MaxValue);
					currentHoldingPlayer = CircuitNode.ClampSignal(currentHoldingPlayer, 0, int.MaxValue);
				}
				catch (Exception e)
				{
					CircuitDebug.LogException(e);
				}
				lastMasterLastHeldPlayerId = lastHeldPlayer;
				lastMasterCurrentHoldingPlayerId = currentHoldingPlayer;
			}
		}
		...
		{
			base.SerializePostEvaluateState(bitPacker);
			bool flag = lastMasterLastHeldPlayerId > 0;
			bitPacker.WriteBool(flag);
			if (flag)
			{
				bitPacker.WriteInt(lastMasterLastHeldPlayerId, 0, int.MaxValue);
			}
		}
		...
		internal override void DeserializePostEvaluateState(BitPacker bitPacker)
		{
			...
			if (value)
			{
				bitPacker.ReadInt(out lastMasterLastHeldPlayerId, 0, int.MaxValue);
			}
			else
			{
				lastMasterLastHeldPlayerId = 0;
			}
		}
	}
