...
namespace RecRoom.Circuits
{
	...
	public class CircuitNodeUuidAttribute : Attribute
	{
		...
		[CompilerGenerated]
		private bool <InternalOnly>k__BackingField;
		...
		public bool InternalOnly
		{
			...
			set
			{
				<InternalOnly>k__BackingField = value;
			}
		}
	...
	public abstract class CircuitNode
	{
		...
		[CompilerGenerated]
		private Action m_NodeModifiedEvent;
		...
		internal virtual bool IsLoopNode => false;
		...
		public event Action NodeModifiedEvent
		{
			...
			add
			{
				Action action = this.m_NodeModifiedEvent;
				Action action2;
				do
				{
					action2 = action;
					Action value2 = (Action)Delegate.Combine(action2, value);
					action = Interlocked.CompareExchange(ref this.m_NodeModifiedEvent, value2, action2);
				}
		...
		internal static void OverrideMaxNodeId(long overrideValue)
		{
			lock (circuitIdLockObj)
			{
				globalMaxCircuitNodeId = overrideValue;
			}
		}
		...
		private void SerializeOutputValues(BitPacker bitPacker)
		{
			...
			{
				int num3 = (int)IOSlots.AllOutputSlots[j];
	...
	public class CircuitNodeGraph : IComparable<CircuitNodeGraph>
	{
		...
		{
			public CircuitNodeGraphCollectionData GraphCollectionData { get; set; }

			public Dictionary<int, int> NodeMapping { get; set; }
		}

		public static List<CircuitNodeGraph> AllGraphs = new List<CircuitNodeGraph>();

		[CompilerGenerated]
		private static UpdateGraph m_TimeUpdate;
		...
		internal static event UpdateGraph TimeUpdate
		{
			...
			add
			{
				UpdateGraph updateGraph = CircuitNodeGraph.m_TimeUpdate;
				UpdateGraph updateGraph2;
				do
				{
					updateGraph2 = updateGraph;
					UpdateGraph value2 = (UpdateGraph)Delegate.Combine(updateGraph2, value);
					updateGraph = Interlocked.CompareExchange(ref CircuitNodeGraph.m_TimeUpdate, value2, updateGraph2);
				}
				while (updateGraph != updateGraph2);
			}
			[CompilerGenerated]
			remove
			{
				...
				{
					updateGraph2 = updateGraph;
					UpdateGraph value2 = (UpdateGraph)Delegate.Remove(updateGraph2, value);
					updateGraph = Interlocked.CompareExchange(ref CircuitNodeGraph.m_TimeUpdate, value2, updateGraph2);
				}
				while (updateGraph != updateGraph2);
			}
		...
		public void DestroySelfAndNodes()
		{
			...
			foreach (CircuitNode item in Nodes.ToList())
			{
				item.Destroy(disconnectLinks: false);
			}
			Destroy();
		...
		public static CircuitNodeGraphCollectionData PersistAll(IList<int> inactiveIds = null)
		{
			CircuitNodeGraphCollectionData circuitNodeGraphCollectionData = new CircuitNodeGraphCollectionData();
			IEnumerable<CircuitNodeGraphData> values = from g in AllGraphs.Where(delegate(CircuitNodeGraph g)
				{
					if (g.IsInternalGraph)
					{
						return false;
					}
					if (inactiveIds != null && g.Nodes.Count == 2)
					{
						foreach (CircuitNode node in g.Nodes)
						{
							if (node != g.RootNode && inactiveIds.Contains(node.NodeId))
							{
								return false;
							}
						}
		...
		public static CircuitNodeGraphCopy CopyNodesToNewGraphs(List<CircuitNode> nodesToCopy)
		{
			...
			foreach (CircuitNode item2 in nodesToCopy)
			{
				if (item2.IsRootNode)
				{
					return null;
			...
			long maxNodeId = CircuitNode.GetMaxNodeId();
			try
			{
				CircuitNodeGraph circuitNodeGraph = new CircuitNodeGraph();
				int nodeId = circuitNodeGraph.RootNode.NodeId;
				Dictionary<int, int> dictionary = new Dictionary<int, int>();
				Dictionary<CircuitNode, CircuitNodeData> dictionary2 = new Dictionary<CircuitNode, CircuitNodeData>();
				Dictionary<int, CircuitNode> dictionary3 = new Dictionary<int, CircuitNode>();
				HashSet<int> hashSet = new HashSet<int>();
				foreach (CircuitNode item3 in nodesToCopy)
				{
					int nodeId2 = item3.NodeGraph.RootNode.NodeId;
					dictionary[nodeId2] = nodeId;
					hashSet.Add(nodeId2);
				}
				foreach (CircuitNode item4 in nodesToCopy)
				{
					CircuitNodeData circuitNodeData = item4.Serialize();
					CircuitNode circuitNode = Serialization.InstantiateNode(circuitNodeData, circuitNodeGraph, reassignNodeIds: true, dictionary);
					dictionary2[circuitNode] = circuitNodeData;
					dictionary3[circuitNode.NodeId] = circuitNode;
				}
				foreach (KeyValuePair<CircuitNode, CircuitNodeData> item5 in dictionary2)
				{
					item5.Key.InitializeInnerData(item5.Value, reassignNodeIds: true, dictionary, copyingSettings: false);
				}
				foreach (KeyValuePair<CircuitNode, CircuitNodeData> item6 in dictionary2)
				{
					foreach (CircuitLinkData item7 in item6.Value.CircuitLinks.ToList())
					{
						if (!dictionary.ContainsKey(item7.SourceNodeId) || !dictionary.ContainsKey(item7.DestNodeId))
						{
							item6.Value.CircuitLinks.Remove(item7);
						}
					}
					item6.Key.DeserializeLinks(item6.Value, dictionary);
				}
				foreach (CircuitNode item8 in nodesToCopy)
				{
					int key = dictionary[item8.NodeId];
					CircuitNode circuitNode2 = dictionary3[key];
					if (circuitNode2.IsLoopNode && circuitNode2.Outputs.Count == 0)
					{
						circuitNode2.Destroy();
						dictionary.Remove(item8.NodeId);
						dictionary3.Remove(key);
					}
				}
				List<CircuitNodeGraph> list = new List<CircuitNodeGraph>();
				HashSet<int> hashSet2 = new HashSet<int>();
				foreach (CircuitNode item9 in nodesToCopy)
				{
					if (dictionary.ContainsKey(item9.NodeId))
					{
						int item = dictionary[item9.NodeId];
						hashSet2.Add(item);
					}
				}
				foreach (CircuitNode item10 in nodesToCopy)
				{
					if (!dictionary.ContainsKey(item10.NodeId))
					{
						continue;
					}
					int num = dictionary[item10.NodeId];
					CircuitNode root = dictionary3[num];
					if (!hashSet2.Contains(num))
					{
						continue;
					}
					bool flag = list.Count > 0;
					CircuitNodeGraph circuitNodeGraph2 = null;
					circuitNodeGraph2 = ((!flag) ? circuitNodeGraph : new CircuitNodeGraph());
					foreach (CircuitNode item11 in GraphTraversal.DFS(root, DFSType.Undirected, ~(LinkTypes.RootToVirtual | LinkTypes.RootToNormal)))
					{
						hashSet2.Remove(item11.NodeId);
						if (flag)
						{
							item11.SetNodeGraph(circuitNodeGraph2, skipMerge: true);
						}
					}
					list.Add(circuitNodeGraph2);
				}
				foreach (int item12 in hashSet)
				{
					dictionary.Remove(item12);
				}
				CircuitNodeGraphCollectionData circuitNodeGraphCollectionData = new CircuitNodeGraphCollectionData();
				foreach (CircuitNodeGraph item13 in list)
				{
					circuitNodeGraphCollectionData.Graphs.Add(item13.Persist());
				}
				foreach (CircuitNodeGraph item14 in list)
				{
					item14.DestroySelfAndNodes();
				}
				return new CircuitNodeGraphCopy
				{
					GraphCollectionData = circuitNodeGraphCollectionData,
					NodeMapping = dictionary
				};
			}
			finally
			{
				CircuitNode.OverrideMaxNodeId(maxNodeId);
			}
		}
	...
	public class GameSetStateNode : PostEvalActionNode
	{
		...
		public event Action StartGame
		{
			...
			add
			{
				Action action = this.m_StartGame;
				Action action2;
				do
				{
					action2 = action;
					Action value2 = (Action)Delegate.Combine(action2, value);
					action = Interlocked.CompareExchange(ref this.m_StartGame, value2, action2);
				}
				while (action != action2);
			}
			[CompilerGenerated]
		...
		public event Action EndGame
		{
			...
			add
			{
				Action action = this.m_EndGame;
				Action action2;
				do
				{
					action2 = action;
					Action value2 = (Action)Delegate.Combine(action2, value);
					action = Interlocked.CompareExchange(ref this.m_EndGame, value2, action2);
				}
				while (action != action2);
			}
			[CompilerGenerated]
	...
	public class ObjectSetTagsNode : CircuitNode
	{
		...
		[CompilerGenerated]
		private ChangeTagsForObjectFunc m_ChangeTagsForObject;
		...
		public event ChangeTagsForObjectFunc ChangeTagsForObject
		{
			...
			add
			{
				ChangeTagsForObjectFunc changeTagsForObjectFunc = this.m_ChangeTagsForObject;
				ChangeTagsForObjectFunc changeTagsForObjectFunc2;
				do
				{
					changeTagsForObjectFunc2 = changeTagsForObjectFunc;
					ChangeTagsForObjectFunc value2 = (ChangeTagsForObjectFunc)Delegate.Combine(changeTagsForObjectFunc2, value);
					changeTagsForObjectFunc = Interlocked.CompareExchange(ref this.m_ChangeTagsForObject, value2, changeTagsForObjectFunc2);
				}
				while (changeTagsForObjectFunc != changeTagsForObjectFunc2);
			}
			[CompilerGenerated]
			remove
			{
				...
				{
					changeTagsForObjectFunc2 = changeTagsForObjectFunc;
					ChangeTagsForObjectFunc value2 = (ChangeTagsForObjectFunc)Delegate.Remove(changeTagsForObjectFunc2, value);
					changeTagsForObjectFunc = Interlocked.CompareExchange(ref this.m_ChangeTagsForObject, value2, changeTagsForObjectFunc2);
				}
				while (changeTagsForObjectFunc != changeTagsForObjectFunc2);
			}
		...
		protected override string GetNodeDescription()
		{
			...
			{
				ChangeModes.Add => "Add Tag", 
				ChangeModes.Defaults => "Reset Tags", 
				ChangeModes.Remove => "Remove Tag", 
				_ => "Change Tags on Object", 
			};
		}
	...
	public class PlayerRoleMappingNode : PostEvalActionNode
	{
		...
		[CompilerGenerated]
		private PlayerModifyRoleFunc m_PlayerAddRole;

		[CompilerGenerated]
		private PlayerModifyRoleFunc m_TeamAddRole;

		[CompilerGenerated]
		private PlayerModifyRoleFunc m_PlayerRemoveRole;

		[CompilerGenerated]
		private PlayerModifyRoleFunc m_TeamRemoveRole;

		[CompilerGenerated]
		private PlayerHasRoleFunc m_PlayerHasRole;
		...
		public event PlayerModifyRoleFunc PlayerAddRole
		{
			...
			add
			{
				PlayerModifyRoleFunc playerModifyRoleFunc = this.m_PlayerAddRole;
				PlayerModifyRoleFunc playerModifyRoleFunc2;
				do
				{
					playerModifyRoleFunc2 = playerModifyRoleFunc;
					PlayerModifyRoleFunc value2 = (PlayerModifyRoleFunc)Delegate.Combine(playerModifyRoleFunc2, value);
					playerModifyRoleFunc = Interlocked.CompareExchange(ref this.m_PlayerAddRole, value2, playerModifyRoleFunc2);
				}
				while (playerModifyRoleFunc != playerModifyRoleFunc2);
			}
			[CompilerGenerated]
			remove
			{
				...
				{
					playerModifyRoleFunc2 = playerModifyRoleFunc;
					PlayerModifyRoleFunc value2 = (PlayerModifyRoleFunc)Delegate.Remove(playerModifyRoleFunc2, value);
					playerModifyRoleFunc = Interlocked.CompareExchange(ref this.m_PlayerAddRole, value2, playerModifyRoleFunc2);
				}
				while (playerModifyRoleFunc != playerModifyRoleFunc2);
			}
		...
		public event PlayerModifyRoleFunc TeamAddRole
		{
			...
			add
			{
				PlayerModifyRoleFunc playerModifyRoleFunc = this.m_TeamAddRole;
				PlayerModifyRoleFunc playerModifyRoleFunc2;
				do
				{
					playerModifyRoleFunc2 = playerModifyRoleFunc;
					PlayerModifyRoleFunc value2 = (PlayerModifyRoleFunc)Delegate.Combine(playerModifyRoleFunc2, value);
					playerModifyRoleFunc = Interlocked.CompareExchange(ref this.m_TeamAddRole, value2, playerModifyRoleFunc2);
				}
				while (playerModifyRoleFunc != playerModifyRoleFunc2);
			}
			[CompilerGenerated]
		...
		public event PlayerModifyRoleFunc PlayerRemoveRole
		{
			...
			add
			{
				PlayerModifyRoleFunc playerModifyRoleFunc = this.m_PlayerRemoveRole;
				PlayerModifyRoleFunc playerModifyRoleFunc2;
				do
				{
					playerModifyRoleFunc2 = playerModifyRoleFunc;
					PlayerModifyRoleFunc value2 = (PlayerModifyRoleFunc)Delegate.Combine(playerModifyRoleFunc2, value);
					playerModifyRoleFunc = Interlocked.CompareExchange(ref this.m_PlayerRemoveRole, value2, playerModifyRoleFunc2);
				}
				while (playerModifyRoleFunc != playerModifyRoleFunc2);
			}
			[CompilerGenerated]
		...
		public event PlayerModifyRoleFunc TeamRemoveRole
		{
			...
			add
			{
				PlayerModifyRoleFunc playerModifyRoleFunc = this.m_TeamRemoveRole;
				PlayerModifyRoleFunc playerModifyRoleFunc2;
				do
				{
					playerModifyRoleFunc2 = playerModifyRoleFunc;
					PlayerModifyRoleFunc value2 = (PlayerModifyRoleFunc)Delegate.Combine(playerModifyRoleFunc2, value);
					playerModifyRoleFunc = Interlocked.CompareExchange(ref this.m_TeamRemoveRole, value2, playerModifyRoleFunc2);
				}
				while (playerModifyRoleFunc != playerModifyRoleFunc2);
			}
			[CompilerGenerated]
		...
		public event PlayerHasRoleFunc PlayerHasRole
		{
			...
			add
			{
				PlayerHasRoleFunc playerHasRoleFunc = this.m_PlayerHasRole;
				PlayerHasRoleFunc playerHasRoleFunc2;
				do
				{
					playerHasRoleFunc2 = playerHasRoleFunc;
					PlayerHasRoleFunc value2 = (PlayerHasRoleFunc)Delegate.Combine(playerHasRoleFunc2, value);
					playerHasRoleFunc = Interlocked.CompareExchange(ref this.m_PlayerHasRole, value2, playerHasRoleFunc2);
				}
				while (playerHasRoleFunc != playerHasRoleFunc2);
			}
			[CompilerGenerated]
			remove
			{
				...
				{
					playerHasRoleFunc2 = playerHasRoleFunc;
					PlayerHasRoleFunc value2 = (PlayerHasRoleFunc)Delegate.Remove(playerHasRoleFunc2, value);
					playerHasRoleFunc = Interlocked.CompareExchange(ref this.m_PlayerHasRole, value2, playerHasRoleFunc2);
				}
				while (playerHasRoleFunc != playerHasRoleFunc2);
			}
	...
	public class PlayerTeamMappingNode : PostEvalActionNode
	{
		...
		[CompilerGenerated]
		private PlayerToTeamIndexFunc m_PlayerToTeamIndex;

		[CompilerGenerated]
		private PlayerSetTeamFunc m_PlayerSetTeam;

		[CompilerGenerated]
		private TeamIndexToPlayerFunc m_TeamIndexToPlayer;

		[CompilerGenerated]
		private GetTeamPlayerCountFunc m_GetTeamPlayerCount;
		...
		public event PlayerToTeamIndexFunc PlayerToTeamIndex
		{
			...
			add
			{
				PlayerToTeamIndexFunc playerToTeamIndexFunc = this.m_PlayerToTeamIndex;
				PlayerToTeamIndexFunc playerToTeamIndexFunc2;
				do
				{
					playerToTeamIndexFunc2 = playerToTeamIndexFunc;
					PlayerToTeamIndexFunc value2 = (PlayerToTeamIndexFunc)Delegate.Combine(playerToTeamIndexFunc2, value);
					playerToTeamIndexFunc = Interlocked.CompareExchange(ref this.m_PlayerToTeamIndex, value2, playerToTeamIndexFunc2);
				}
				while (playerToTeamIndexFunc != playerToTeamIndexFunc2);
			}
			[CompilerGenerated]
			remove
			{
				...
				{
					playerToTeamIndexFunc2 = playerToTeamIndexFunc;
					PlayerToTeamIndexFunc value2 = (PlayerToTeamIndexFunc)Delegate.Remove(playerToTeamIndexFunc2, value);
					playerToTeamIndexFunc = Interlocked.CompareExchange(ref this.m_PlayerToTeamIndex, value2, playerToTeamIndexFunc2);
				}
				while (playerToTeamIndexFunc != playerToTeamIndexFunc2);
			}
		...
		public event PlayerSetTeamFunc PlayerSetTeam
		{
			...
			add
			{
				PlayerSetTeamFunc playerSetTeamFunc = this.m_PlayerSetTeam;
				PlayerSetTeamFunc playerSetTeamFunc2;
				do
				{
					playerSetTeamFunc2 = playerSetTeamFunc;
					PlayerSetTeamFunc value2 = (PlayerSetTeamFunc)Delegate.Combine(playerSetTeamFunc2, value);
					playerSetTeamFunc = Interlocked.CompareExchange(ref this.m_PlayerSetTeam, value2, playerSetTeamFunc2);
				}
				while (playerSetTeamFunc != playerSetTeamFunc2);
			}
			[CompilerGenerated]
			remove
			{
				...
				{
					playerSetTeamFunc2 = playerSetTeamFunc;
					PlayerSetTeamFunc value2 = (PlayerSetTeamFunc)Delegate.Remove(playerSetTeamFunc2, value);
					playerSetTeamFunc = Interlocked.CompareExchange(ref this.m_PlayerSetTeam, value2, playerSetTeamFunc2);
				}
				while (playerSetTeamFunc != playerSetTeamFunc2);
			}
		...
		public event TeamIndexToPlayerFunc TeamIndexToPlayer
		{
			...
			add
			{
				TeamIndexToPlayerFunc teamIndexToPlayerFunc = this.m_TeamIndexToPlayer;
				TeamIndexToPlayerFunc teamIndexToPlayerFunc2;
				do
				{
					teamIndexToPlayerFunc2 = teamIndexToPlayerFunc;
					TeamIndexToPlayerFunc value2 = (TeamIndexToPlayerFunc)Delegate.Combine(teamIndexToPlayerFunc2, value);
					teamIndexToPlayerFunc = Interlocked.CompareExchange(ref this.m_TeamIndexToPlayer, value2, teamIndexToPlayerFunc2);
				}
				while (teamIndexToPlayerFunc != teamIndexToPlayerFunc2);
			}
			[CompilerGenerated]
			remove
			{
				...
				{
					teamIndexToPlayerFunc2 = teamIndexToPlayerFunc;
					TeamIndexToPlayerFunc value2 = (TeamIndexToPlayerFunc)Delegate.Remove(teamIndexToPlayerFunc2, value);
					teamIndexToPlayerFunc = Interlocked.CompareExchange(ref this.m_TeamIndexToPlayer, value2, teamIndexToPlayerFunc2);
				}
				while (teamIndexToPlayerFunc != teamIndexToPlayerFunc2);
			}
		...
		public event GetTeamPlayerCountFunc GetTeamPlayerCount
		{
			...
			add
			{
				GetTeamPlayerCountFunc getTeamPlayerCountFunc = this.m_GetTeamPlayerCount;
				GetTeamPlayerCountFunc getTeamPlayerCountFunc2;
				do
				{
					getTeamPlayerCountFunc2 = getTeamPlayerCountFunc;
					GetTeamPlayerCountFunc value2 = (GetTeamPlayerCountFunc)Delegate.Combine(getTeamPlayerCountFunc2, value);
					getTeamPlayerCountFunc = Interlocked.CompareExchange(ref this.m_GetTeamPlayerCount, value2, getTeamPlayerCountFunc2);
				}
				while (getTeamPlayerCountFunc != getTeamPlayerCountFunc2);
			}
			[CompilerGenerated]
			remove
			{
				...
				{
					getTeamPlayerCountFunc2 = getTeamPlayerCountFunc;
					GetTeamPlayerCountFunc value2 = (GetTeamPlayerCountFunc)Delegate.Remove(getTeamPlayerCountFunc2, value);
					getTeamPlayerCountFunc = Interlocked.CompareExchange(ref this.m_GetTeamPlayerCount, value2, getTeamPlayerCountFunc2);
				}
				while (getTeamPlayerCountFunc != getTeamPlayerCountFunc2);
			}
		...
		protected override string GetNodeDescription()
		{
			return PlayerIdMapMode switch
			{
				PlayerIdMapModes.SetPlayerTeam => "Set Player Team", 
				PlayerIdMapModes.GetTeamPlayerCount => "Get Team Player Count", 
				PlayerIdMapModes.TeamToPlayer => "Team to Player", 
				_ => "Player to Team", 
			};
		}
	...
	public class ObjectPlayerMappingNode : CircuitNode
	{
		...
		private static readonly OutputMetadata[] outputMeta = new OutputMetadata[2]
		{
			...
			{
				OutputSlot = OutputSlot.G,
				Description = "Currently Holding Player"
			}
		};

		[CompilerGenerated]
		private ObjectToHoldingPlayerIdFunc m_ObjectToHoldingPlayer;
		...
		public event ObjectToHoldingPlayerIdFunc ObjectToHoldingPlayer
		{
			...
			add
			{
				ObjectToHoldingPlayerIdFunc objectToHoldingPlayerIdFunc = this.m_ObjectToHoldingPlayer;
				ObjectToHoldingPlayerIdFunc objectToHoldingPlayerIdFunc2;
				do
				{
					objectToHoldingPlayerIdFunc2 = objectToHoldingPlayerIdFunc;
					ObjectToHoldingPlayerIdFunc value2 = (ObjectToHoldingPlayerIdFunc)Delegate.Combine(objectToHoldingPlayerIdFunc2, value);
					objectToHoldingPlayerIdFunc = Interlocked.CompareExchange(ref this.m_ObjectToHoldingPlayer, value2, objectToHoldingPlayerIdFunc2);
				}
				while (objectToHoldingPlayerIdFunc != objectToHoldingPlayerIdFunc2);
			}
			[CompilerGenerated]
			remove
			{
				...
				{
					objectToHoldingPlayerIdFunc2 = objectToHoldingPlayerIdFunc;
					ObjectToHoldingPlayerIdFunc value2 = (ObjectToHoldingPlayerIdFunc)Delegate.Remove(objectToHoldingPlayerIdFunc2, value);
					objectToHoldingPlayerIdFunc = Interlocked.CompareExchange(ref this.m_ObjectToHoldingPlayer, value2, objectToHoldingPlayerIdFunc2);
				}
				while (objectToHoldingPlayerIdFunc != objectToHoldingPlayerIdFunc2);
			}
		...
		protected override void _EvaluateNode(bool isMasterClient)
		{
			base._EvaluateNode(isMasterClient);
			int inputSignal = GetInputSignal(InputSlot.R);
			bool num = CircuitNode.SignalToBool(inputSignal);
			int lastHeldPlayerId = 0;
			int currentHoldingPlayerId = 0;
			if (num && isMasterClient)
			{
				MasterCalcOutput(inputSignal, out lastHeldPlayerId, out currentHoldingPlayerId);
				SetContinuousOutput(OutputSlot.R, lastHeldPlayerId);
				SetContinuousOutput(OutputSlot.G, currentHoldingPlayerId);
			}
		}
		...
		{
			lastHeldPlayerId = 0;
			currentHoldingPlayerId = 0;
			try
			{
				this.ObjectToHoldingPlayer(objectId, out lastHeldPlayerId, out currentHoldingPlayerId);
				lastHeldPlayerId = CircuitNode.ClampSignal(lastHeldPlayerId, 0, int.MaxValue);
				currentHoldingPlayerId = CircuitNode.ClampSignal(currentHoldingPlayerId, 0, int.MaxValue);
			}
			catch (Exception e)
			{
				CircuitDebug.LogException(e);
	...
	public class RandomNode : CircuitNode
	{
		...
		private static readonly OutputMetadata[] outputMeta = new OutputMetadata[1]
		{
			...
			{
				OutputSlot = OutputSlot.R,
				Description = "Random # from [Green to Blue)"
			}
		};
		...
		protected override string GetNodeDescription()
		{
			return "Random # from [Green to Blue)";
		}
	...
	public class SelectorNode : CircuitNode
	{
		...
		protected override string GetNodeDescription()
		{
			if (EvalMode == EvalModes.First)
			{
				return "Output First True";
			}
			return "Output All True";
		}
	...
	public class CombinatorNode : CircuitNode
	{
		...
		private static readonly InputMetadata[] TwoInputsMeta = new InputMetadata[3]
		{
			new InputMetadata
			{
				InputSlot = InputSlot.R
		...
		private static readonly InputMetadata[] ThreeInputsMeta = new InputMetadata[4]
		{
			new InputMetadata
			{
				InputSlot = InputSlot.R
		...
		protected override InputMetadata[] GetInputMetadata()
		{
			...
			{
				return TwoInputsMeta;
			}
			return ThreeInputsMeta;
		}
		...
		protected override string GetNodeDescription()
		{
			string arg = ((CombinatorMode == CombinatorModes.Add) ? $" {GetModeSymbol(CombinatorMode)} Blue" : string.Empty);
			return $"Red {GetModeSymbol(CombinatorMode)} Green{arg}";
		}
	...
	public class LoopNode : VirtualNode
	{
		...
		internal override bool IsLoopNode => true;
	...
	public class WaveNode : CircuitNode
	{
		...
		protected override string GetNodeDescription()
		{
			...
			{
				WaveMode.Sine => "Sine Wave", 
				WaveMode.Cosine => "Cosine Wave", 
				WaveMode.Square => "Square Wave", 
				WaveMode.Triangle => "Triangle Wave", 
				WaveMode.Sawtooth => "Sawtooth Wave", 
				_ => "Wave Function", 
			};
		}
...
namespace RecRoom.Circuits.PlayerStats
{
	...
	public class PlayerGetStatNode : SourceNodeBase
	{
		...
		[CompilerGenerated]
		private GetPlayerStatFunction m_GetPlayerStat;
		...
		public event GetPlayerStatFunction GetPlayerStat
		{
			...
			add
			{
				GetPlayerStatFunction getPlayerStatFunction = this.m_GetPlayerStat;
				GetPlayerStatFunction getPlayerStatFunction2;
				do
				{
					getPlayerStatFunction2 = getPlayerStatFunction;
					GetPlayerStatFunction value2 = (GetPlayerStatFunction)Delegate.Combine(getPlayerStatFunction2, value);
					getPlayerStatFunction = Interlocked.CompareExchange(ref this.m_GetPlayerStat, value2, getPlayerStatFunction2);
				}
				while (getPlayerStatFunction != getPlayerStatFunction2);
			}
			[CompilerGenerated]
			remove
			{
				...
				{
					getPlayerStatFunction2 = getPlayerStatFunction;
					GetPlayerStatFunction value2 = (GetPlayerStatFunction)Delegate.Remove(getPlayerStatFunction2, value);
					getPlayerStatFunction = Interlocked.CompareExchange(ref this.m_GetPlayerStat, value2, getPlayerStatFunction2);
				}
				while (getPlayerStatFunction != getPlayerStatFunction2);
			}
		...
		protected override string GetNodeDescription()
		{
			return PlayerStat switch
			{
				CircuitPlayerStat.PlayerStat_6 => "Get Stat 6", 
				CircuitPlayerStat.PlayerStat_5 => "Get Stat 5", 
				CircuitPlayerStat.PlayerStat_4 => "Get Stat 4", 
				CircuitPlayerStat.PlayerStat_3 => "Get Stat 3", 
				CircuitPlayerStat.PlayerStat_2 => "Get Stat 2", 
				CircuitPlayerStat.PlayerStat_1 => "Get Stat 1", 
				_ => "Get Health", 
			};
		}
	...
	public class PlayerSetStatNode : PostEvalActionNode
	{
		...
		[CompilerGenerated]
		private SetPlayerStatFunction m_SetPlayerStat;
		...
		public event SetPlayerStatFunction SetPlayerStat
		{
			...
			add
			{
				SetPlayerStatFunction setPlayerStatFunction = this.m_SetPlayerStat;
				SetPlayerStatFunction setPlayerStatFunction2;
				do
				{
					setPlayerStatFunction2 = setPlayerStatFunction;
					SetPlayerStatFunction value2 = (SetPlayerStatFunction)Delegate.Combine(setPlayerStatFunction2, value);
					setPlayerStatFunction = Interlocked.CompareExchange(ref this.m_SetPlayerStat, value2, setPlayerStatFunction2);
				}
				while (setPlayerStatFunction != setPlayerStatFunction2);
			}
			[CompilerGenerated]
			remove
			{
				...
				{
					setPlayerStatFunction2 = setPlayerStatFunction;
					SetPlayerStatFunction value2 = (SetPlayerStatFunction)Delegate.Remove(setPlayerStatFunction2, value);
					setPlayerStatFunction = Interlocked.CompareExchange(ref this.m_SetPlayerStat, value2, setPlayerStatFunction2);
				}
				while (setPlayerStatFunction != setPlayerStatFunction2);
			}
		...
		protected override string GetNodeDescription()
		{
			return PlayerStat switch
			{
				CircuitPlayerStat.PlayerStat_6 => "Set Stat 6", 
				CircuitPlayerStat.PlayerStat_5 => "Set Stat 5", 
				CircuitPlayerStat.PlayerStat_4 => "Set Stat 4", 
				CircuitPlayerStat.PlayerStat_3 => "Set Stat 3", 
				CircuitPlayerStat.PlayerStat_2 => "Set Stat 2", 
				CircuitPlayerStat.PlayerStat_1 => "Set Stat 1", 
				_ => "Set Health", 
			};
		}
...
namespace RecRoom.Circuits.GameStats
{
	...
	public class GameGetScoreNode : SourceNodeBase
	{
		...
		[CompilerGenerated]
		private GetTeamScoreFunction m_GetTeamScore;
		...
		public event GetTeamScoreFunction GetTeamScore
		{
			...
			add
			{
				GetTeamScoreFunction getTeamScoreFunction = this.m_GetTeamScore;
				GetTeamScoreFunction getTeamScoreFunction2;
				do
				{
					getTeamScoreFunction2 = getTeamScoreFunction;
					GetTeamScoreFunction value2 = (GetTeamScoreFunction)Delegate.Combine(getTeamScoreFunction2, value);
					getTeamScoreFunction = Interlocked.CompareExchange(ref this.m_GetTeamScore, value2, getTeamScoreFunction2);
				}
				while (getTeamScoreFunction != getTeamScoreFunction2);
			}
			[CompilerGenerated]
			remove
			{
				...
				{
					getTeamScoreFunction2 = getTeamScoreFunction;
					GetTeamScoreFunction value2 = (GetTeamScoreFunction)Delegate.Remove(getTeamScoreFunction2, value);
					getTeamScoreFunction = Interlocked.CompareExchange(ref this.m_GetTeamScore, value2, getTeamScoreFunction2);
				}
				while (getTeamScoreFunction != getTeamScoreFunction2);
			}
	...
	public class GameSetScoreNode : PostEvalActionNode
	{
		...
		[CompilerGenerated]
		private SetTeamScoreFunction m_SetTeamScore;
		...
		public event SetTeamScoreFunction SetTeamScore
		{
			...
			add
			{
				SetTeamScoreFunction setTeamScoreFunction = this.m_SetTeamScore;
				SetTeamScoreFunction setTeamScoreFunction2;
				do
				{
					setTeamScoreFunction2 = setTeamScoreFunction;
					SetTeamScoreFunction value2 = (SetTeamScoreFunction)Delegate.Combine(setTeamScoreFunction2, value);
					setTeamScoreFunction = Interlocked.CompareExchange(ref this.m_SetTeamScore, value2, setTeamScoreFunction2);
				}
				while (setTeamScoreFunction != setTeamScoreFunction2);
			}
			[CompilerGenerated]
			remove
			{
				...
				{
					setTeamScoreFunction2 = setTeamScoreFunction;
					SetTeamScoreFunction value2 = (SetTeamScoreFunction)Delegate.Remove(setTeamScoreFunction2, value);
					setTeamScoreFunction = Interlocked.CompareExchange(ref this.m_SetTeamScore, value2, setTeamScoreFunction2);
				}
				while (setTeamScoreFunction != setTeamScoreFunction2);
			}
...
namespace RecRoom.Circuits.Utils
{
	...
	public static class CircuitDebug
	{
		...
		[CompilerGenerated]
		private static LogEvent m_OnLogEvent;

		[CompilerGenerated]
		private static LogExceptionEvent m_OnLogExceptionEvent;
		...
		public static event LogEvent OnLogEvent
		{
			...
			add
			{
				LogEvent logEvent = CircuitDebug.m_OnLogEvent;
				LogEvent logEvent2;
				do
				{
					logEvent2 = logEvent;
					LogEvent value2 = (LogEvent)Delegate.Combine(logEvent2, value);
					logEvent = Interlocked.CompareExchange(ref CircuitDebug.m_OnLogEvent, value2, logEvent2);
				}
				while (logEvent != logEvent2);
			}
			[CompilerGenerated]
			remove
			{
				...
				{
					logEvent2 = logEvent;
					LogEvent value2 = (LogEvent)Delegate.Remove(logEvent2, value);
					logEvent = Interlocked.CompareExchange(ref CircuitDebug.m_OnLogEvent, value2, logEvent2);
				}
				while (logEvent != logEvent2);
			}
		...
		public static event LogExceptionEvent OnLogExceptionEvent
		{
			...
			add
			{
				LogExceptionEvent logExceptionEvent = CircuitDebug.m_OnLogExceptionEvent;
				LogExceptionEvent logExceptionEvent2;
				do
				{
					logExceptionEvent2 = logExceptionEvent;
					LogExceptionEvent value2 = (LogExceptionEvent)Delegate.Combine(logExceptionEvent2, value);
					logExceptionEvent = Interlocked.CompareExchange(ref CircuitDebug.m_OnLogExceptionEvent, value2, logExceptionEvent2);
				}
				while (logExceptionEvent != logExceptionEvent2);
			}
			[CompilerGenerated]
			remove
			{
				...
				{
					logExceptionEvent2 = logExceptionEvent;
					LogExceptionEvent value2 = (LogExceptionEvent)Delegate.Remove(logExceptionEvent2, value);
					logExceptionEvent = Interlocked.CompareExchange(ref CircuitDebug.m_OnLogExceptionEvent, value2, logExceptionEvent2);
				}
				while (logExceptionEvent != logExceptionEvent2);
			}
