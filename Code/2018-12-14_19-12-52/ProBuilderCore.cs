...
[assembly: InternalsVisibleTo("ProBuilderMeshOps")]
...
internal class ChangelogEntry
{
	...
	public pb_VersionInfo versionInfo => m_VersionInfo;
...
public static class pb_Constant
{
	...
	public static Material DefaultMaterial => pb_Material.DefaultMaterial;
}
[Serializable]
...
internal class pb_EdgeConnection : IEquatable<pb_EdgeConnection>
{
	...
	public override string ToString()
	{
		return face.ToString() + " : " + pb_Util.ToString(edges);
	}
...
internal class pb_FaceRebuildData
{
	...
	public override string ToString()
	{
		return $"{pb_Util.ToString(vertices)}\n{pb_Util.ToString(sharedIndices)}";
	}
...
public static class pb_Material
{
	...
	internal static Material TriggerMaterial => (Material)Resources.Load("Materials/Trigger", typeof(Material));

	internal static Material ColliderMaterial => (Material)Resources.Load("Materials/Collider", typeof(Material));

	[Obsolete("NoDraw is no longer supported.")]
	internal static Material NoDrawMaterial => (Material)Resources.Load("Materials/NoDraw", typeof(Material));
...
public static class pb_Math
{
	...
	public static Vector2 ReflectPoint(Vector2 point, Vector2 a, Vector2 b)
	{
		...
		float num = Mathf.Sin(Vector2.Angle(vector, point - a) * ((float)Math.PI / 180f)) * Vector2.Distance(point, a);
	...
	internal static bool PointInPolygon(Vector2[] polygon, Vector2 point, int[] indices = null)
	{
		...
		if (num % 2 != 0)
		{
			UnityEngine.Debug.LogError("PointInPolygon requires polygon indices be divisible by 2!");
			return false;
		}
	...
	public static void NormalTangentBitangent(pb_Object pb, pb_Face face, out Vector3 normal, out Vector3 tangent, out Vector3 bitangent)
	{
		...
		{
			UnityEngine.Debug.LogWarning("Cannot find normal / tangent / bitangent for face with < 3 indices.");
			normal = Vector3.zero;
...
public class pb_Object : MonoBehaviour
{
	...
	private static Action<pb_Object> m_onDestroyObject;

	[CompilerGenerated]
	[DebuggerBrowsable(DebuggerBrowsableState.Never)]
	private static Action<pb_Object> m_onElementSelectionChanged;
	...
	public bool hasUv2 => msh.uv2 != null && msh.uv2.Length == vertexCount;
	...
	public int faceCount => (_quads != null) ? _quads.Length : 0;

	public int vertexCount => (_vertices != null) ? _vertices.Length : 0;

	public int triangleCount => (_quads != null) ? _quads.Sum((pb_Face x) => x.indices.Length) : 0;
	...
	public static event Action<pb_Object> onDestroyObject
	{
		add
		{
			Action<pb_Object> action = pb_Object.m_onDestroyObject;
			Action<pb_Object> action2;
			do
			{
				action2 = action;
				action = Interlocked.CompareExchange(ref pb_Object.m_onDestroyObject, (Action<pb_Object>)Delegate.Combine(action2, value), action);
			}
		...
		remove
		{
			...
			{
				action2 = action;
				action = Interlocked.CompareExchange(ref pb_Object.m_onDestroyObject, (Action<pb_Object>)Delegate.Remove(action2, value), action);
			}
			while (action != action2);
		}
	}
	...
	internal static event Action<pb_Object> onElementSelectionChanged
	{
		add
		{
			Action<pb_Object> action = pb_Object.m_onElementSelectionChanged;
			Action<pb_Object> action2;
			do
			{
				action2 = action;
				action = Interlocked.CompareExchange(ref pb_Object.m_onElementSelectionChanged, (Action<pb_Object>)Delegate.Combine(action2, value), action);
			}
			while (action != action2);
		}
		remove
		{
			...
			{
				action2 = action;
				action = Interlocked.CompareExchange(ref pb_Object.m_onElementSelectionChanged, (Action<pb_Object>)Delegate.Remove(action2, value), action);
			}
			while (action != action2);
		}
	}
	...
	internal void SetSelectedFaces(IEnumerable<int> selected)
	{
		...
		{
			ref pb_Edge reference = ref m_SelectedEdges[num2];
			reference = array[num2];
		}
	...
	private void GeometryWithPoints(Vector3[] v)
	{
		...
		for (int i = 0; i < v.Length; i += 4)
		{
			array[i / 4] = new pb_Face(new int[6]
			{
				...
				i + 3,
				i + 2
			}, pb_Material.DefaultMaterial, new pb_UV(), 0, -1, -1, manualUV: false);
		}
		SetVertices(v);
...
public struct pb_PickerOptions
{
	...
	public static pb_PickerOptions Default => k_Default;
}
internal static class pb_Picking
{
	public static Dictionary<pb_Object, HashSet<int>> PickVerticesInRect(Camera cam, Rect rect, IList<pb_Object> selectable, pb_PickerOptions options, float pixelsPerPoint = 1f)
	{
		...
		{
			return pb_SelectionPicker.PickVerticesInRect(cam, rect, selectable, doDepthTest: true, (int)((float)cam.pixelWidth / pixelsPerPoint), (int)((float)cam.pixelHeight / pixelsPerPoint));
		}
	...
	public static Dictionary<pb_Object, HashSet<pb_Face>> PickFacesInRect(Camera cam, Rect rect, IList<pb_Object> selectable, pb_PickerOptions options, float pixelsPerPoint = 1f)
	{
		...
		foreach (pb_Object item in selectable)
		{
			...
			{
				ref Vector3 reference = ref array[i];
				reference = cam.ScreenToGuiPoint(cam.WorldToScreenPoint(transform.TransformPoint(vertices[i])), pixelsPerPoint);
			}
	...
	public static Dictionary<pb_Object, HashSet<pb_Edge>> PickEdgesInRect(Camera cam, Rect rect, IList<pb_Object> selectable, pb_PickerOptions options, float pixelsPerPoint = 1f)
	{
		if (options.depthTest && options.rectSelectMode == pb_RectSelectMode.Partial)
		{
			return pb_SelectionPicker.PickEdgesInRect(cam, rect, selectable, doDepthTest: true, (int)((float)cam.pixelWidth / pixelsPerPoint), (int)((float)cam.pixelHeight / pixelsPerPoint));
		}
...
internal class pb_PreferenceDictionary : ScriptableObject, ISerializationCallbackReceiver, pb_IHasDefault
{
	...
	public bool HasKey<T>(string key)
	{
		...
		UnityEngine.Debug.LogWarning($"HasKey<{typeof(T).ToString()}>({key}) not valid preference type.");
		return false;
	}
	...
	public T Get<T>(string key, T fallback = default(T))
	{
		...
		{
			UnityEngine.Debug.LogWarning($"Get<{typeof(T).ToString()}>({key}) not valid preference type.");
		}
		return fallback;
	}
	...
	public void Set<T>(string key, T value)
	{
		...
		{
			UnityEngine.Debug.LogWarning($"Set<{typeof(T).ToString()}>({key}, {value.ToString()}) not valid preference type.");
		}
	}
...
internal class pb_Renderable : ScriptableObject
{
	...
	public static pb_Renderable CreateInstance(Mesh InMesh, Material InMaterial, Transform transform = null)
	{
		...
		pb_Renderable2.material = InMaterial;
	...
	{
		pb_Renderable pb_Renderable2 = CreateInstance(new Mesh(), null);
	...
	public void Render()
	{
		...
		for (int subMeshCount = mesh.subMeshCount; i < subMeshCount; i++)
		{
			...
			{
				Graphics.DrawMeshNow(mesh, (!(transform != null)) ? Matrix4x4.identity : transform.localToWorldMatrix, i);
			}
		}
	}
}
internal static class pb_SelectionPicker
{
	...
	private static Texture2D RenderWithReplacementShader(Camera camera, Shader shader, string tag, int width = -1, int height = -1)
	{
		...
		Texture2D texture2D = new Texture2D(num, num2, textureFormat, mipmap: false, linear: false);
...
public static class pb_ShapeGenerator
{
	...
	public static pb_Object CreateShape(pb_ShapeType shape)
	{
		...
		{
			pb_Object2 = StairGenerator(new Vector3(2f, 2.5f, 4f), 6, buildSides: true);
		}
		if (shape == pb_ShapeType.CurvedStair)
		{
			pb_Object2 = CurvedStairGenerator(2f, 2.5f, 2f, 180f, 8, buildSides: true);
		}
		...
		{
			pb_Object2 = ArchGenerator(180f, 2f, 1f, 1f, 9, insideFaces: true, outsideFaces: true, frontFaces: true, backFaces: true, endCaps: true);
		}
		if (shape == pb_ShapeType.Icosahedron)
		{
			pb_Object2 = IcosahedronGenerator(0.5f, 2, weldVertices: true, manualUvs: false);
		}
		if (shape == pb_ShapeType.Torus)
		{
			pb_Object2 = TorusGenerator(12, 16, 1f, 0.3f, InSmooth: true, 360f, 360f);
		}
		if (pb_Object2 == null)
		{
			pb_Object2 = CubeGenerator(Vector3.one);
	...
	public static pb_Object CubeGenerator(Vector3 size)
	{
		...
		{
			ref Vector3 reference = ref array[i];
			reference = Vector3.Scale(k_CubeVertices[k_CubeTriangles[i]], size);
		}
	...
	public static pb_Object CylinderGenerator(int axisDivisions, float radius, float height, int heightCuts, int smoothing = -1)
	{
		...
		for (int l = 0; l < heightCuts + 1; l++)
		{
			for (int m = 0; m < axisDivisions * 4; m += 4)
			{
				...
				int num8 = num5 + 2;
				int num9 = num5 + 3;
				array3[num4++] = new pb_Face(new int[6] { num6, num7, num8, num7, num9, num8 }, pb_Material.DefaultMaterial, new pb_UV(), smoothing, -1, -1, manualUV: false);
			}
		}
	...
	public static pb_Object IcosahedronGenerator(float radius, int subdivisions, bool weldVertices = true, bool manualUvs = true)
	{
		...
		{
			ref Vector3 reference = ref array[i];
			reference = k_IcosphereVertices[k_IcosphereTriangles[i]].normalized * radius;
			ref Vector3 reference2 = ref array[i + 1];
			reference2 = k_IcosphereVertices[k_IcosphereTriangles[i + 1]].normalized * radius;
			ref Vector3 reference3 = ref array[i + 2];
			reference3 = k_IcosphereVertices[k_IcosphereTriangles[i + 2]].normalized * radius;
		}
...
internal class pb_Shortcut
{
	...
	public pb_Shortcut(string str)
	{
		...
		{
			UnityEngine.Debug.LogWarning("Failed parsing shortcut: " + str);
		}
	}
...
internal static class pb_Spline
{
	...
	internal static void Extrude(IList<Vector3> points, float radius, int radiusRows, bool closeLoop, bool smooth, ref pb_Object target, IList<Quaternion> pointRotations = null)
	{
...
public class pb_VersionInfo : IEquatable<pb_VersionInfo>, IComparable<pb_VersionInfo>, IComparable
{
	...
	public VersionType type => m_Type;

	public string metadata => m_Metadata;

	public string date => m_Date;

	public pb_VersionInfo MajorMinorPatch => new pb_VersionInfo(major, minor, patch);
...
internal class pb_VertexConnection : IEquatable<pb_VertexConnection>
{
	...
	public override string ToString()
	{
		return face.ToString() + " : " + pb_Util.ToString(indices);
	}
