	...
	public class DelayNode : CircuitNode
	{
		...
		protected override void _EvaluateNode(bool isMasterClient)
		{
			...
			if (isMasterClient)
			{
				...
				{
					float val = (float)GetInputSignal(InputSlot.G) / 10f;
					int delay = (int)(1000f * Math.Max(0f, Math.Min(3600f, val)));
	...
	public class TimerNode : CircuitNode
	{
		...
		protected override void _EvaluateNode(bool isMasterClient)
		{
			...
			if (isMasterClient)
			{
				...
				int inputSignal = GetInputSignal(InputSlot.G);
				inputTime = (float)inputSignal / 10f;
				UpdateOutputs();
			}
		}
		...
		{
			timeRemaining = (int)(1000f * Math.Max(0f, Math.Min(3600f, inputTime)));
...
namespace RecRoom.Circuits.Utils
{
	...
	public static class CircuitDebug
	{
		...
		public static void Log(CircuitLogLevel level, string message, CircuitLogLevel CircuitLogLevel = CircuitLogLevel.Warning)
		{
		...
		public static void NodeLog(int nodeId, string format, CircuitLogLevel CircuitLogLevel = CircuitLogLevel.Warning, params object[] args)
		{
