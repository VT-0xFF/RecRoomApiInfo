...
{
	private const int MAX_SMOOTH_GROUPS = 24;
	...
	public void RefreshUV(IEnumerable<pb_Face> facesToRefresh)
	{
		...
		foreach (pb_Face item in facesToRefresh)
		{
			...
			if (item != null && !item.manualUV)
			{
				...
				dictionary.Add((item.textureGroup > 0) ? item.textureGroup : num--, new List<pb_Face> { item });
			}
		}
		...
		num = 0;
		Vector3[] verts = (flag ? base.transform.ToWorldSpace(vertices) : null);
		foreach (KeyValuePair<int, List<pb_Face>> item2 in dictionary)
		{
			...
			{
				vector = pb_Projection.FindBestPlane(_vertices, indices).normal;
			}
			else
			{
	...
	public void RefreshNormals()
	{
		...
		for (int k = 0; k < sharedIndices.Length; k++)
		{
			...
			for (int m = 0; m < sharedIndices[k].array.Length; m++)
			{
				...
				if (array3[num3] >= 1 && array3[num3] <= 24)
				{
					array[array3[num3]].x += normals[num3].x;
					array[array3[num3]].y += normals[num3].y;
					array[array3[num3]].z += normals[num3].z;
					array2[array3[num3]] += 1f;
				}
			}
			for (int n = 0; n < sharedIndices[k].array.Length; n++)
			{
				int num4 = sharedIndices[k].array[n];
				if (array3[num4] >= 1 && array3[num4] <= 24)
				{
					normals[num4].x = array[array3[num4]].x / array2[array3[num4]];
					normals[num4].y = array[array3[num4]].y / array2[array3[num4]];
					normals[num4].z = array[array3[num4]].z / array2[array3[num4]];
				}
			}
		}
...
namespace ProBuilder2.Common
{
	...
	public static class pb_Projection
	{
		...
		{
			int num = ((indices == null || indices.Length < 1) ? verts.Length : indices.Length);
		...
		public static Plane FindBestPlane(Vector3[] points, int[] indices = null)
		{
			...
			int num7 = (flag ? indices.Length : points.Length);
			Vector3 zero = Vector3.zero;
			Vector3 zero2 = Vector3.zero;
			for (int i = 0; i < num7; i++)
			{
				zero.x += points[flag ? indices[i] : i].x;
				zero.y += points[flag ? indices[i] : i].y;
				zero.z += points[flag ? indices[i] : i].z;
			}
			zero.x /= num7;
			zero.y /= num7;
			zero.z /= num7;
			...
			{
				Vector3 vector = points[flag ? indices[j] : j] - zero;
...
namespace ProBuilder2.Common
{
	...
	public class pb_Bounds2D
	{
		...
		public static Vector2 Center(Vector2[] points, int length = -1)
		{
			...
			float num3 = 0f;
			float num4 = 0f;
			int num5 = ((length < 1) ? points.Length : length);
			num = points[0].x;
			num3 = points[0].y;
			num2 = num;
			num4 = num3;
...
namespace ProBuilder2.Common
{
	...
	public class pb_WingedEdge : IEquatable<pb_WingedEdge>, IEnumerable
	{
		...
		{
			Dictionary<int, int> dictionary = ((sharedIndexLookup == null) ? pb.sharedIndices.ToDictionary() : sharedIndexLookup);
